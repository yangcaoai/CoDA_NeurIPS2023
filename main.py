...
from engine import evaluate, train_one_epoch, show_boxes, save_box_points, crop_image, calculate_wrong_class, save_novel, save_seen, save_novel_with_class, wandb_log_boxes, save_clip_feature
...
from utils.logger import Logger, WandbLogger
...
def make_args_parser():
    ...
    parser.add_argument("--less_tokens_num", default=98, type=int)
    parser.add_argument("--virtual_cate_num", default=400, type=int)
    parser.add_argument("--virtual_sample_num", default=150, type=int)
    parser.add_argument("--final_lr", default=1e-6, type=float)
    parser.add_argument("--lr_scheduler", default="cosine", type=str)
    parser.add_argument("--weight_decay", default=0.1, type=float)
    parser.add_argument("--reset_epoch_cnt", default=0, type=int)
    ...
    parser.add_argument("--if_load_clip_feat", default=False, action="store_true")
    ...
    parser.add_argument("--reload_iters", default=1400, type=int)
    parser.add_argument("--if_clip_more_prompts", default=False, action="store_true")
    parser.add_argument("--if_reload_data", default=False, action="store_true")
    parser.add_argument("--if_not_paste_generated_image", default=False, action="store_true")
    parser.add_argument("--if_not_need_2d_box", default=False, action="store_true")
    ...
    parser.add_argument("--if_pre_text", default=False, action="store_true")
    parser.add_argument("--if_use_lvis_text", default=False, action="store_true")
    ...
    parser.add_argument("--box2d_driven_iou", default=1, type=float)
    parser.add_argument("--box2d_driven_bg_iou", default=1, type=float)
    ...
    parser.add_argument("--online_3d_nod_global_novel", default=False,
                        action="store_true")
    parser.add_argument("--online_3d_nod_global_novel_wo_bg_cate", default=False,
                        action="store_true")
    parser.add_argument("--online_3d_nod_global_novel_wo_denoising", default=False,
                        action="store_true")
    parser.add_argument("--online_3d_nod_global_novel_wo_denoising_simple_bg", default=False,
                        action="store_true")
    parser.add_argument("--online_nms_update_save_novel_label_clip_driven_with_cate_confidence_with_2d_pos_prior", default=False, action="store_true")
    parser.add_argument("--online_nms_update_save_novel_label_clip_driven_with_cate_confidence_with_2d_pos_prior_combined_label", default=False, action="store_true")
    parser.add_argument("--online_nms_with_2d_pos_prior_combined_label_2d_label", default=False, action="store_true")
    parser.add_argument("--if_average_score", default=False, action="store_true")
    parser.add_argument("--begin_combine_score_epoch", default=1000000, type=int)
    parser.add_argument("--online_nms_update_save_novel_label_clip_and_head_driven_with_cate_confidence", default=False, action="store_true")
    ...
    parser.add_argument("--if_get_virtual_embedding", default=False, action="store_true")
    parser.add_argument("--if_get_global_embedding", default=False, action="store_true")
    parser.add_argument("--online_nms_update_save_novel_label_with_prob", default=False, action="store_true")
    parser.add_argument("--adopt_2d_box_guide_objness", default=False, action="store_true")
    ...
    parser.add_argument("--loss_predicted_region_embed_contrast_weight", default=0, type=float)
    parser.add_argument(
        "--loss_no_object_weight", default=0.2, type=float
    )  # "no object" or "background" class for detection
    parser.add_argument(
        "--loss_2d_no_object_weight", default=0.2, type=float
    )  # "no object" or "background" class for detection
    ...
    parser.add_argument("--loss_predicted_region_embed_contrast_with_virtual_image_weight", default=0, type=float)
    ...
    parser.add_argument("--loss_predicted_region_embed_l1_virtual_weight", default=0, type=float)
    parser.add_argument("--loss_predicted_region_embed_l1_global_weight", default=0, type=float)
    parser.add_argument("--loss_feat_seen_softmax_weakly_loss_with_novel_cate_confi_skip_virtual_weight", default=0, type=float)
    ...
    parser.add_argument("--loss_virtual_object_text_alignment_weight", default=0, type=float)
    ...
    parser.add_argument("--loss_2d_sem_cls_softmax_skip_none_gt_sample_weight", default=0, type=float)
    parser.add_argument("--loss_sem_cls_softmax_skip_none_gt_sample_before_aug_weight", default=0, type=float)
    ...
    ##### Training #####
    parser.add_argument('--strategies', nargs='+', default=['top', 'side'], help='List of placement strategies')
    ...
    parser.add_argument("--max_virtual_obj", default=5, type=int)
    parser.add_argument("--max_global_obj", default=5, type=int)
    parser.add_argument("--min_points_num_of_virtual_obj", default=2048, type=int)
    parser.add_argument("--min_points_num_of_global_obj", default=2048, type=int)
    ...
    parser.add_argument("--if_with_bg_cate", default=False,  action="store_true")
    parser.add_argument("--if_adopt_caption_0313", default=False,  action="store_true")
    parser.add_argument("--if_adopt_caption_scannet_0415", default=False,  action="store_true")
        ##### Testing #####
    ...
    parser.add_argument("--save_clip_feat_only", default=False, action="store_true")
    ...
    parser.add_argument("--save_clip_feat_dir", default=None, type=str)
    ...
    parser.add_argument("--combine_clip_distill", default=False, action="store_true")
    ...
    parser.add_argument("--if_input_test_text", default=False, action="store_true")
    parser.add_argument("--ob_type", default="mean", type=str)
    ...
    parser.add_argument("--prj_name", default='cmp_cvpr', type=str)
    ...
    parser.add_argument("--topk_text", default=20, type=int)
    parser.add_argument("--max_num_obj", default=64, type=int)

    ##### Distributed Training #####
    parser.add_argument("--ngpus", default=1, type=int)

    parser.add_argument("--if_use_clip_v2", default=False, action="store_true")
    parser.add_argument("--dist_url", default="tcp://localhost:12345", type=str)

    parser.add_argument("--if_test_f1_score", default=False, action="store_true")
    return parser
...
def do_train(
    ...
    if is_primary() and args.on_cloud:
        logger = WandbLogger(prj_name=args.prj_name, exp_name=args.exp_name, notes=args.checkpoint_dir, tags=args.checkpoint_dir, dir=args.checkpoint_dir+'/', configs=args) #()
        # logger = Logger(args.checkpoint_dir + '/log/')
    else:
        logger = None
    ...
    reset_epoch_cnt = args.reset_epoch_cnt
    ...
    # # -----------------just see eval-------------------
        ...
        curr_iter = all_epoch * len(dataloaders["train"])
        ...
        if (
            ...
            and all_epoch % args.save_separate_checkpoint_every_epoch == 0
        ):
        ...
        # if (all_epoch % args.real_eval_every_epoch == 0) or all_epoch == (args.max_epoch - 1):
            ...
            if is_primary():
                log_boxes(args, model, model_no_ddp, None, dataset_config, dataloaders, args.show_dir, args.if_after_nms, epoch=all_epoch, logger=logger, curr_iter=curr_iter)
                # print(
                #     'python cloud_tools/mox.py -s /home/ma-user/modelarts/user-job-dir/ov_3d_det_release/%s/wandb -t s3://bucket-cneast3/caoyang/code/3d_vision/%s/wandb' % (
                #         args.checkpoint_dir, args.checkpoint_dir))
                # os.system(
                #     'python cloud_tools/mox.py -s /home/ma-user/modelarts/user-job-dir/ov_3d_det_release/%s/wandb -t s3://bucket-cneast3/caoyang/code/3d_vision/%s/wandb' % (
                #         args.checkpoint_dir, args.checkpoint_dir))

        # if (all_epoch % args.reset_epoch_periodically == 0):
        #     if is_primary() and args.on_cloud:
        #         print('python cloud_tools/mox.py -s /cache/data/make_data_scannet200_cluster/scannet_frames_25k_200cls_noveltrain_pseudo_labels_setting0 -t s3://bucket-cneast3/caoyang/code/3d_vision/%s/scannet_frames_25k_200cls_noveltrain_pseudo_labels_setting0_%04d' % (
        #             args.checkpoint_dir, all_epoch))
        #         os.system('python cloud_tools/mox.py -s /cache/data/make_data_scannet200_cluster/scannet_frames_25k_200cls_noveltrain_pseudo_labels_setting0 -t s3://bucket-cneast3/caoyang/code/3d_vision/%s/scannet_frames_25k_200cls_noveltrain_pseudo_labels_setting0_%04d' % (
        #             args.checkpoint_dir, all_epoch))

        #         print('python cloud_tools/mox.py -s /cache/data/sunrgbd_v1_revised_0415/sunrgbd_pc_bbox_votes_50k_v1_all_classes_revised_0415_noveltrain_global_novel_setting0 -t s3://bucket-cneast3/caoyang/code/3d_vision/%s/sunrgbd_pc_bbox_votes_50k_v1_all_classes_revised_0415_noveltrain_global_novel_setting0_%04d' % (
        #             args.checkpoint_dir, all_epoch))
        #         os.system('python cloud_tools/mox.py -s /cache/data/sunrgbd_v1_revised_0415/sunrgbd_pc_bbox_votes_50k_v1_all_classes_revised_0415_noveltrain_global_novel_setting0 -t s3://bucket-cneast3/caoyang/code/3d_vision/%s/sunrgbd_pc_bbox_votes_50k_v1_all_classes_revised_0415_noveltrain_global_novel_setting0_%04d' % (
        #             args.checkpoint_dir, all_epoch))
...
def log_boxes(args, model, model_no_ddp, criterion, dataset_config, dataloaders, out_dir, if_after_nms, epoch=-1, logger=None, curr_iter=0):
...
def save_clip_feat(args, model, model_no_ddp, criterion, dataset_config, dataloaders, out_dir, if_after_nms):
    if args.test_ckpt is None or not os.path.isfile(args.test_ckpt):
        f"Please specify a test checkpoint using --test_ckpt. Found invalid value {args.test_ckpt}"
        sys.exit(1)
    ...
    curr_iter = 0
    # print('before show_boxes')
    ap_calculator = save_clip_feature(
        args,
        epoch,
        model,
        criterion,
...
def test_model(args, model, model_no_ddp, criterion, dataset_config, dataloaders, log_file=None, if_mini=False, real_cmp_test_dataset_config=None):
    ...
    if args.if_test_f1_score:
        metrics = ap_calculator.compute_metrics_more()
        metric_str = ap_calculator.metrics_to_str_more(metrics)
    else:
        metrics = ap_calculator.compute_metrics()
        metric_str = ap_calculator.metrics_to_str(metrics)
...
def main(local_rank, args):
    ...
    elif args.save_clip_feat_only:
        os.makedirs(args.save_clip_feat_dir, exist_ok=True)
        save_clip_feat(args, model, model_no_ddp, criterion, dataset_config, dataloaders, args.save_seen_dir, args.if_after_nms)
