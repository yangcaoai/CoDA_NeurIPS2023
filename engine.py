...
def train_one_epoch(
    ...
    for batch_idx, batch_data_label in enumerate(dataset_loader):
        ...
        outputs = model(batch_data_label, curr_epoch=curr_epoch, args=args, all_epoch=all_epoch)
        ...
        if is_primary() and curr_iter % args.log_every == 0:
            ...
            #
            # if "loss_sem_cls_softmax_skip_none_gt_sample_before_aug" in loss_dict_reduced.keys():
            #     loss_sem_cls_softmax_skip_none_gt_sample_before_aug = loss_dict_reduced["loss_sem_cls_softmax_skip_none_gt_sample_before_aug"]
            # else:
            #     loss_sem_cls_softmax_skip_none_gt_sample_before_aug = -1.0
            #
            #
...
def wandb_camera_cord_to_image_plane(inputs, outputs, out_dir=None, if_padding_input=True, args=None, logger=None, epoch=-1):
    sem_cls_probs = outputs['sem_cls_prob']
    objectness_prob = outputs['objectness_prob']
    gt_classes = inputs['gt_box_sem_cls_label'].cpu().numpy()
    #pred_sem_cls_prob = np.max(sem_cls_probs, -1)  # B,num_proposal. the max prob
    ...
    for idx_batch in range(len(calib_name_list)):
        ...
        for idx_box in range(len(box_center_gt_list)):
            ...
            box_class = dataconfig.eval_class2type[int(gt_classes[idx_batch][idx_box])]
            # print(gt_classes[idx_batch][idx_box])
            if int(gt_classes[idx_batch][idx_box]) > 9:
                if_has_novel = True
                cv2.putText(img_draw2d_box, '%s' % (box_class), (max(int(xmin), 15),
        ...
        #     continue
        # cv2.imwrite(os.path.join(out_dir, im_name+'_3d_to_2d_box_gt.png'), img_draw2d_box)
        Img = wandb.Image(img_draw2d_box, caption=im_name+'_gt.png')  # here!!!
        logger.log_image({"GT": Img})
        # trans_affine = trans_affine_batch[idx_batch]
        img_draw2d_box = img.copy()
        img_draw2d_box = cv2.cvtColor(img_draw2d_box, cv2.COLOR_RGB2BGR)
        for idx_box in range(len(box_center_list)):
            ...
            #     box_class = dataconfig.superclass2type[pred_sem_cls]
            # else:
            box_class = dataconfig.eval_class2type[pred_sem_cls]
        ...
        # print(os.path.join(out_dir, im_name+'_3d_to_2d_box_pred.png'))
        # cv2.imwrite(os.path.join(out_dir, im_name+'_3d_to_2d_box_pred.png'), img_draw2d_box)
        Img = wandb.Image(img_draw2d_box, caption="epoch:{}".format(epoch))  # here!!!
        logger.log_image({"Pred": Img})
...
def show_boxes(
    ...
    for batch_idx, batch_data_label in enumerate(dataset_loader):
        ...
        for sample_idx in range(len(point_cloud_rgb_this_batch)):
            ...
            #     continue
            # just comments for a while
            color_pc_path = '/home/xuhang/Data/3d_indoor/sunrgb_d/sunrgbd_v1_0415/sunrgbd_pc_bbox_votes_50k_v1_all_classes_0415_val/%s' % im_name_tmp
            # read_pc_data(color_pc_path, out_path=out_dir)
            tmp_point_clouds_np = point_cloud_rgb_this_batch[sample_idx].cpu().numpy()
            # write_ply_rgb(tmp_point_clouds_np[:, 0:6], out_dir+'/'+'%s_pc.ply' % im_name_tmp)
            color_point(tmp_point_clouds_np[:, 0:6], save_path=out_dir+'/'+'%s_pc.ply' % im_name_tmp)
            print('YesYesYesYesYesYesYesYesYesYesYesYesYesYesYes')
            print(out_dir+'/'+'%s_pc.ply' % im_name_tmp)
        ...
        #print(boxes_this_image.shape)
            ...
            if boxes_this_image.shape[0]>0:
                ...
                tmp_box_here = boxes_this_image.copy()
                tmp_box_here = tmp_box_here[:, :7]
                if args.dataset_name.find('sunrgbd') != -1:
                    tmp_box_here[:, -1] = tmp_box_here[:, -1]  * -1
                tmp_box_here = tmp_box_here[boxes_cls_max>0.5,:]

                # print(tmp_box[:, -1])
                # print('********************')
                # print(tmp_box.shape)
                if tmp_box_here.shape[0]>0:
                    write_oriented_bbox(tmp_box_here, out_dir+'/'+'%s_pred.ply' % im_name_tmp)
...
def wandb_log_boxes(
    ...
    for batch_idx, batch_data_label in enumerate(dataset_loader):
        ...
        #
        # }
        outputs = model(batch_data_label, if_real_test=True, args=args)
        # Compute loss
        loss_str = ""
        if criterion is not None:
            loss, loss_dict = criterion(outputs, batch_data_label)
        ...
        if if_after_nms:
            # print('1111111111111111111111111111111')
            wandb_camera_cord_to_image_plane(batch_data_label, output_nms, out_dir, args=args, logger=logger, epoch=curr_epoch)
        else:
            wandb_camera_cord_to_image_plane(batch_data_label, outputs['outputs'], out_dir, args=args, logger=logger, epoch=curr_epoch)
...
def save_seen(
    ...
    for batch_idx, batch_data_label in enumerate(dataset_loader):
        ...
        for sample_idx in range(len(point_cloud_rgb_this_batch)):
            im_name_tmp = os.path.basename(im_name_list[sample_idx])[:-4]
            # if '/home/xuhang/Data/3d_indoor/sunrgb_d/sunrgbd_trainval/image/007426.jpg' == im_name_list[sample_idx]:
            #     print(1)
...
def save_clip_feature(
    ...
    for batch_idx, batch_data_label in enumerate(dataset_loader):
        ...
        #
        # }
        outputs = model(batch_data_label, if_save_clip_feat=True)
        # Compute loss
        # loss_str = ""
        # if criterion is not None:
        #     loss, loss_dict = criterion(outputs, batch_data_label)
        ...
        # Memory intensive as it gathers point cloud GT tensor across all ranks
        outputs["outputs"] = all_gather_dict(outputs["outputs"])
        clip_feature = outputs["outputs"]['clip_feats'].cpu().numpy()
        # print(batch_data_label.keys())
        ...
        # pred_obb_this_batch, gt_this_batch, point_cloud_rgb_this_batch = ap_calculator.step_meter_show(outputs, batch_data_label)
        # print('44444444444444444444444')
        for sample_idx in range(clip_feature.shape[0]):
            im_name_tmp = os.path.basename(im_name_list[sample_idx])[:-4]
            # if '/home/xuhang/Data/3d_indoor/sunrgb_d/sunrgbd_trainval/image/007426.jpg' == im_name_list[sample_idx]:
            #     print(1)
            ...
            # pred_obb_this_image = pred_obb_this_batch[sample_idx]
            # new_boxes = np.zeros((len(pred_obb_this_image), 8))
            clip_feat_name = args.save_clip_feat_dir + '/' + im_name_tmp + '_clip_feat.npy'
            save_clip_feat = clip_feature[sample_idx]
            np.save(clip_feat_name, save_clip_feat)
            # for idx_box in range(len(pred_obb_this_image)):
            #     box_name = im_name_tmp + '_%04d_seen_bbox_feat_info.npy' % idx_box
            #     new_boxes[idx_box][:7] = pred_obb_this_image[idx_box][0][3][:7].cpu().numpy()
            #     new_boxes[idx_box][-1] = -1
...
def evaluate(
    ...
    for batch_idx, batch_data_label in enumerate(dataset_loader):
        ...
        outputs = model(batch_data_label, if_real_test=if_real_test, if_cmp_class=if_cmp_class, args=args)
