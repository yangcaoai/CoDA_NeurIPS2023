...
from .helpers_vit import build_model_with_cfg, resolve_pretrained_cfg, named_apply, adapt_input_conv, checkpoint_seq
...
default_cfgs = {
    ...
    'vit_small_patch16_224': _cfg(
        ...
        file='outputs/S_16-i21k-300ep-lr_0.001-aug_light1-wd_0.03-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.03-res_224.npz'
        ),
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple) (B, self.num_heads, N, C//self.num_heads)
...
class MultiBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.id_layer > 7:
            ...
            self.ln_1_cross = norm_layer(dim)
            self.ln_2_cross = norm_layer(dim)
            self.mlp_cross = Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop)

    def cross_attention(self, query: torch.Tensor, key=None, value=None):
        # self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        res = self.cross_attn(query, key, value, need_weights=False)
        return res[0]

    def forward(self, input_list):
        x = input_list[0]
        pc_feat = input_list[1]
        ...
        if self.id_layer > 7:
            im_feat = x[:, 232:, :]
            im_feat = im_feat.permute(1, 0, 2)
            pc_feat = pc_feat + self.cross_attention(query=self.ln_1_cross(pc_feat), key=im_feat, value=im_feat)
            pc_feat = pc_feat + self.mlp_cross(self.ln_2_cross(pc_feat))

        return [x, pc_feat]

# class TwoModalBlock(nn.Module):
#
#     def __init__(
#             self,
#             dim,
...
class TwoModalBlock(nn.Module):
    ...
    def forward(self, input_list):
        im_feat = input_list[0]
        text_feat = input_list[1]

        im_feat = im_feat + self.drop_path1(self.ls1(self.attn(self.norm1(im_feat))))
        im_feat = im_feat + self.drop_path2(self.ls2(self.mlp(self.norm2(im_feat))))

        # if self.id_layer > 7:
            # im_feat = x
        # im_feat = im_feat.permute(1, 0, 2)
        im_feat = im_feat + self.cross_attention(query=self.ln_1_cross(im_feat), key=text_feat, value=text_feat)
        im_feat = im_feat + self.mlp_cross(self.ln_2_cross(im_feat))

        return [im_feat, text_feat]
...
class VisionTwoModalTransformer(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        # self.linear_pc = nn.Linear(512, 768)
        # self.linear_text = nn.Linear(512, 384)
        # Classifier Head
    ...
    def forward_features(self, x, text_features):
        ...
        # x_text = torch.cat((text_features_clip, x), dim=1)
        # x = self._pos_embed(x)
        x_image = self.norm_pre(x)
        # pc_features = self.linear_pc(pc_features)

        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.blocks, x)
        else:
            out_list = self.blocks([x_image, text_features])
        x = out_list[0]
        x = self.norm(x)
        return x
    ...
    def forward(self, image_features, text_features, if_pre_text=False):
        ori_dim = image_features.shape[1]
        image_resized_batch = image_features.reshape((image_features.shape[0], -1, image_features.shape[-1]))
        image_resized_batch = image_resized_batch.permute(1, 0, 2)
        if if_pre_text:
            text_features = text_features.permute(1, 0, 2)
        else:
            text_features = text_features.unsqueeze(1).repeat(1, image_resized_batch.shape[1], 1)
        x = self.forward_features(image_resized_batch, text_features)
        x = x.permute(1, 0, 2)
        x = x.reshape((image_features.shape[0], ori_dim, ori_dim, image_features.shape[-1]))
        # x = self.forward_head(x)
        return x

class VisionMultiModalTransformer(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if class_token else None
        embed_len = num_patches if no_embed_class else num_patches + self.num_prefix_tokens
        self.pos_embed = nn.Parameter(torch.randn(1, 232+embed_len, embed_dim) * .02)
        self.pos_drop = nn.Dropout(p=drop_rate)
        self.norm_pre = norm_layer(embed_dim) if pre_norm else nn.Identity()
    ...
    def forward_features(self, x, text_features_clip, pc_features):
        x = self.patch_embed(x)
        text_features_clip = text_features_clip.unsqueeze(0).repeat(x.shape[0], 1, 1)
        text_features_clip = self.linear_text(text_features_clip)
        x_text = torch.cat((text_features_clip, x), dim=1)
        x_text = self._pos_embed(x_text)
        x_text = self.norm_pre(x_text)
        # pc_features = self.linear_pc(pc_features)

        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.blocks, x)
        else:
            out_list = self.blocks([x_text, pc_features])
        x = out_list[1]
        x = self.norm(x)
        return x
    ...
    def forward(self, image_resized_batch, text_features_clip, pc_features):
        x = self.forward_features(image_resized_batch, text_features_clip, pc_features)
        # x = self.forward_head(x)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, img_crop_resized, text_features_clip, pc_features):
        x = self.forward_features(img_crop_resized)
        # x = self.forward_head(x)
        return x
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    if model.pos_embed.shape[1] != 428 and model.pos_embed.shape[1] != 196:
...
def _create_multi_modal_vision_transformer(variant, pretrained=False, npz_path=None, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')

    pretrained_cfg = resolve_pretrained_cfg(variant, pretrained_cfg=kwargs.pop('pretrained_cfg', None))
    model = build_model_with_cfg(
        VisionMultiModalTransformer, variant, pretrained,
        pretrained_cfg=pretrained_cfg,
        pretrained_filter_fn=checkpoint_filter_fn,
        pretrained_custom_load='npz' in pretrained_cfg['url'],
        npz_path=npz_path,
...
def _create_two_modal_vision_transformer(variant, pretrained=False, npz_path=None, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')

    pretrained_cfg = resolve_pretrained_cfg(variant, pretrained_cfg=kwargs.pop('pretrained_cfg', None))
    model = build_model_with_cfg(
        VisionTwoModalTransformer, variant, pretrained,
        pretrained_cfg=pretrained_cfg,
        pretrained_filter_fn=checkpoint_filter_fn,
        pretrained_custom_load='npz' in pretrained_cfg['url'],
        npz_path=npz_path,
