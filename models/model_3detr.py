...
from utils.box_util import flip_axis_to_depth
...
ALL_SUPERCLASS_PATH_LVIS = 'datasets/lvis_1204.npy'
ALL_SUPERCLASS_PATH_Image2Caption = 'datasets/final_list_wo_color_select_main_word_only_odd_more_cate.npy'
ALL_SUPERCLASS_PATH_Image2Caption_Scannet = 'datasets/scannet_final_list_wo_color_select_main_word_only_odd_more_cate.npy'
ALL_SUPERCLASS_PATH = 'datasets/obj365_categories.npy'
...
class Model3DETRPredictedBoxDistillationHead(nn.Module):
    ...
    def __init__(
        ...
        self.if_adopt_caption_0313 = args.if_adopt_caption_0313
        if self.if_with_clip_train:
            ...
            if args.dataset_name.find('scannet') != -1:
                ...
                if self.if_clip_more_prompts:
                    ...
                    all_classes_keys = all_classes_keys[
                        train_idx_list]  # [:dataset_config.num_semcls] just seen classes during training.
            else:
                ...
                if self.if_clip_more_prompts:
                    ...
                    all_classes_keys = list(all_classes.keys())[
                                       :args.train_range_max]  # [:dataset_config.num_semcls] just seen classes during training.
            ...
            if self.if_clip_superset:
                if self.if_adopt_caption_0313:
                    superset_all_classes = np.load(ALL_SUPERCLASS_PATH_Image2Caption, allow_pickle=True)
                else:
                    superset_all_classes = np.load(ALL_SUPERCLASS_PATH_LVIS, allow_pickle=True)
                ...
                if (not self.if_adopt_caption_0313) and ALL_SUPERCLASS_PATH_LVIS.find('lvis_1204') != -1:
                    superset_all_classes_keys = list(superset_all_classes)[1:]  # the  first one is "name"
                    tmp_superset = ['a photo of a ' + i.replace('_', ' ').lower() + ' in the scene' for i in
                                    superset_all_classes_keys]
                else:
                    superset_all_classes_keys = list(superset_all_classes)
                    tmp_superset = ['a photo of a ' + i.replace('_', ' ').replace('/', ' or ').lower() + ' in the scene'
                                    for i in
                                    superset_all_classes_keys]
                ...
                for i in tmp_superset:
                    ...
                    if i not in self.superset_all_classes_keys:
                        # print(i)
                        self.superset_all_classes_keys.append(i)
                        self.lvis_map_super[idx_lvis] = idx_super
                        idx_super += 1
                    else:
                        tmp_idx_super = self.superset_all_classes_keys.index(i)
                        self.lvis_map_super[idx_lvis] = tmp_idx_super
                        cnt_same += 1
                    idx_lvis += 1

                # self.if_use_lvis_text = args.if_use_lvis_text
                # print('=====================if use lvis text=====================')
                # print(self.if_use_lvis_text)
                # if self.if_use_lvis_text:
                #     superset_all_classes_lvis = np.load(ALL_SUPERCLASS_PATH_LVIS, allow_pickle=True)
                #     superset_all_classes_lvis_keys = list(superset_all_classes_lvis)[1:]  # the  first one is "name"
                #     tmp_superset_lvis = ['a photo of a ' + i.replace('_', ' ').lower() + ' in the scene' for i in
                #                          superset_all_classes_lvis_keys]
                #     cnt_same = 0
                #     for i in tmp_superset_lvis:
                #         #     # text_item = 'a photo of a ' + i.replace('_', ' ').lower() + ' in the scene'
                #         if i not in self.superset_all_classes_keys:
                #             # print(i)
                #             self.superset_all_classes_keys.append(i)
                ...
                assert len(self.superset_all_classes_keys) == 1201 or len(
                    self.superset_all_classes_keys) == 1216 or len(self.superset_all_classes_keys) == 232 or len(
                    self.superset_all_classes_keys) == 1203 or len(self.superset_all_classes_keys) == 369 or len(
                    self.superset_all_classes_keys) == 1387 or len(
                    self.superset_all_classes_keys) == 935 or len(
                    self.superset_all_classes_keys) == 1009 # remember to change here here3

                print(self.superset_all_classes_keys)
                print(len(self.superset_all_classes_keys))

            self.clip_model, self.preprocess_for_tensor = clip.load("ViT-B/16",
                                                                    # "./CLIP/pretrain_models/RN50x64.pt",
            ...
            self.cmp_text_features_fg_norm = self.cmp_text_features_fg / self.cmp_text_features_fg.norm(dim=1,
                                                                                                        keepdim=True)
            ...
            # # ############################ train use
            # self.test_clip_model, self.test_preprocess_for_tensor = clip.load("ViT-B/16", #"./CLIP/pretrain_models/RN50x64.pt", #"./CLIP/pretrain_models/ViT-L-14.pt",
            #                                                                   device=self.device,
            #                                                                   download_root='./CLIP/pretrain_models/',
            #                                                                   if_transform_tensor=True)
            #
        ...
        print(
            '=========================if online_nms_update_save_novel_label_clip_driven_with_cate_confidence_with_2d_pos_prior_combined_label')
        self.online_nms_update_save_novel_label_clip_driven_with_cate_confidence_with_2d_pos_prior_combined_label = args.online_nms_update_save_novel_label_clip_driven_with_cate_confidence_with_2d_pos_prior_combined_label
        self.save_objectness = args.save_objectness
        print(self.online_nms_update_save_novel_label_clip_driven_with_cate_confidence_with_2d_pos_prior_combined_label)
        self.box2d_iou_thres = args.box2d_iou_thres
        print(self.box2d_iou_thres)
        self.begin_combine_score_epoch = args.begin_combine_score_epoch
        print(self.begin_combine_score_epoch)
        self.if_average_score = args.if_average_score
        print('=========================if_average_score=========================')
        print(self.if_average_score)
        ...
        print('=========================if online_3d_nod_global_novel===============================')
        self.online_3d_nod_global_novel = args.online_3d_nod_global_novel
        self.save_objectness = args.save_objectness
        print(self.online_3d_nod_global_novel)
        print(self.save_objectness)
        self.online_nms_update_save_epoch = args.online_nms_update_save_epoch
        print(self.online_nms_update_save_epoch)
        self.clip_driven_keep_thres = args.clip_driven_keep_thres
        ...
        print('==========================if online_nms_with_2d_pos_prior_combined_label_2d_label')
        self.online_nms_with_2d_pos_prior_combined_label_2d_label = args.online_nms_with_2d_pos_prior_combined_label_2d_label
        ...
        print('=====================if_get_virtual_embedding========================')
        self.if_get_virtual_embedding = args.if_get_virtual_embedding
        print(self.if_get_virtual_embedding)
        print('=====================adopt_2d_box_guide_objness======================')
        self.adopt_2d_box_guide_objness = args.adopt_2d_box_guide_objness
        print(self.adopt_2d_box_guide_objness)
        self.box2d_driven_iou = args.box2d_driven_iou
        self.ob_type = args.ob_type
        print('=====================if_get_global_embedding========================')
        self.if_get_global_embedding = args.if_get_global_embedding
        print(self.if_get_global_embedding)
    ...
    def build_mlp_heads(self, dataset_config, decoder_dim, mlp_dropout):
        ...
        # box2d_guidecls_head = mlp_func(output_dim=2)
        ...
        mlp_heads = [
            ...
            # ("box2d_guide_cls_head", box2d_guidecls_head),
    ...
    def clip_to_class_training(self, inputs, outputs, thres_obj=0.05, if_use_gt_box=False, if_expand_box=False,
        ...
        for idx_batch in range(batch_size_data):
            ...
            for idx_box in range(box_num_data):
                ...
                # print(idx_box)
                if if_padding_input:
                    corners_3d_image, corners_3d, d = self.dataset_util.compute_box_3d_offset_tensor(box_center,
                                                                                                     box_angle,
                                                                                                     box_size,
                                                                                                     calib, x_offset,
                                                                                                     y_offset)
                    ...
                    corners_3d_image, corners_3d = self.dataset_util.compute_box_3d_resize(box_center, box_angle,
                                                                                           box_size,
                                                                                           calib, trans_mtx)
    ...
    def get_predicted_box_clip_embedding_nms_iou_save_keep_clip_driven_with_cate_confidence(self, inputs, outputs,
                                                                                            thres_obj=0.05,
                                                                                            if_use_gt_box=False,
                                                                                            if_expand_box=False,
                                                                                            if_padding_input=True,
        ...
        center_2dpoints, depth = self.dataset_util.project_3dpoint_to_2dpoint_corners_tensor(gt_center.to(torch.double),
                                                                                             # project the 3d boxes to 2d axis
                                                                                             K_tensor=K_tensor,
        ...
        if (not if_test) and (
            ...
            save_box_info = torch.zeros(
                (outputs["center_unnormalized"].shape[0], outputs["center_unnormalized"].shape[1], 10),
                # (center(3), size(3), angle, class, cate_prob, objectness)
                device=self.device)
        ...
        for idx_batch in range(center_2dpoints.shape[0]):
                    ...
                    if (curr_epoch % self.online_nms_update_save_epoch == 0):
                        for maybe_idx in range(maybe_condition.shape[0]):
                            ...
                            if maybe_item:
                                ...
                                save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][7] = maybe_max_idx[
                                    maybe_idx]
                                save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][8] = maybe_max_score[
                                    maybe_idx]
                                save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][9] = scores[
                                    idx_maybe_novel_box_list[maybe_idx]]  # update the novel box info
            ...
            select_list = np.random.choice(self.box_idx_list, self.distillation_box_num,
                                           replace=False)  # random select N boxes for distillation
            ...
            proposal_features_batch = self.clip_model.encode_image(
                image_proposal_batch)  # get the 2D clip color image embedding
    ...
    def get_3d_nod_global_novel(self, inputs, outputs,
                                                                                            thres_obj=0.05,
                                                                                            if_use_gt_box=False,
                                                                                            if_expand_box=False,
                                                                                            if_padding_input=True,
        ...
        rot_array = inputs['rot_array'].unsqueeze(1)
        rot_angle = inputs['rot_angle'].unsqueeze(1)
        ori_pc_rgbd = inputs['point_clouds_rgb_ori'].detach().cpu().numpy()
        global_obj_num = inputs['global_obj_num'][0].item()
        # pred_box_corners = outputs["box_corners"].detach().clone()
        # back the augmentation for point cloud
        gt_center = gt_center_ori * scale_array  # [:, :2, :]
        gt_center = torch.matmul(gt_center, rot_array)
        if 'zx_flip_array' in inputs.keys():
            ...
            gt_center[:, :, :, 1] = gt_center[:, :, :, 1] * zx_flip_array
        gt_center[:, :, :, 0] = gt_center[:, :, :, 0] * flip_array  # [:, :2, :]
        device_id = gt_center_ori.device.index
        
        if (not if_test) and (
                curr_epoch % self.online_nms_update_save_epoch == 0):  # and curr_epoch >= 540:
            ori_center_unnormalized = outputs["center_unnormalized"].detach().clone() * inputs['scale_array']
            ori_size_unnormalized = outputs["size_unnormalized"].detach().clone() * inputs['scale_array']
            # pred_box_corners = pred_box_corners * inputs['scale_array']

            ori_center_unnormalized = torch.matmul(ori_center_unnormalized, inputs['rot_array'])
            # pred_box_corners = torch.matmul(pred_box_corners, inputs['rot_array'])
            ori_angle_continuous = outputs["angle_continuous"].detach().clone() + inputs['rot_angle'].unsqueeze(-1)

            if 'zx_flip_array' in inputs.keys():
                ori_center_unnormalized[:, :, 1] = ori_center_unnormalized[:, :, 1] * inputs['zx_flip_array']
                # pred_box_corners[:, :, :, 1] = pred_box_corners[:, :, :, 1] * inputs['zx_flip_array']
                ori_angle_continuous = torch.where(inputs['zx_flip_array'] < 0, (math.pi - ori_angle_continuous),
                                                   ori_angle_continuous)

            ori_center_unnormalized[:, :, 0] = ori_center_unnormalized[:, :, 0] * inputs['flip_array']
            # pred_box_corners[:, :, :, 0] = pred_box_corners[:, :, :, 0] * inputs['flip_array']
            ori_angle_continuous = torch.where(inputs['flip_array'] < 0, (math.pi - ori_angle_continuous),
                                               ori_angle_continuous)
        ...
        for idx_batch in range(center_2dpoints.shape[0]):
            if (not if_test) and (curr_epoch % self.online_nms_update_save_epoch == 0):  # and curr_epoch >= 540:
                ...
                if len(proposal_maybe_novel_feature_list) > 0:
                    ...
                    tmp_ori_pc = ori_pc_rgbd[idx_batch]
                    global_save_path = inputs['global_pool_path'][idx_batch]
                    # write_ply_rgb(tmp_ori_pc, global_save_path + '_all_pc.ply') # save global pool
                    if (not if_test) and (curr_epoch % self.online_nms_update_save_epoch == 0):
                        for maybe_idx in range(maybe_condition.shape[0]):
                            maybe_item = maybe_condition[maybe_idx]
                            if maybe_item:
                                novel_idx_list_final_select.append(idx_maybe_novel_box_list[maybe_idx])
                                ...
                                # tmp_pred_box = pred_box_corners[idx_batch][idx_maybe_novel_box_list[maybe_idx]] this is wrong, because it causes unalignment.
                                tmp_pred_box_batch = self.box_processor.box_parametrization_to_corners_np(
                                    save_box_info[:, :, :3], save_box_info[:, :, 3:6], save_box_info[:, :, 6]
                                )
                                tmp_pred_box = tmp_pred_box_batch[idx_batch][idx_maybe_novel_box_list[maybe_idx]]
                                tmp_im = proposal_maybe_novel_feature_list[maybe_idx][0,:,:,:].permute(1, 2, 0).detach().cpu().numpy()
                                box3d = flip_axis_to_depth(tmp_pred_box)
                                pc_tmp, _ = extract_pc_in_box3d(tmp_ori_pc, box3d)
                                
                                tmp_save_box = save_box_info[idx_batch, idx_maybe_novel_box_list[maybe_idx], :8]
                                if len(pc_tmp) > 500:  #
                                    box_path = global_save_path+'%09d_%04d_box.npy' % (maybe_idx + global_obj_num, device_id)
                                    pc_path = global_save_path+'%09d_%04d_pc.npy' % (maybe_idx + global_obj_num, device_id)
                                    im_path = global_save_path+'%09d_%04d_im.png' % (maybe_idx + global_obj_num, device_id)
                                    np.save(pc_path, pc_tmp)
                                    # write_ply_rgb(pc_tmp, global_save_path+'_%04d_pc.ply' % maybe_idx)
                                    # box_save_tmp = save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][:7][np.newaxis, :]
                                    # box_save_tmp[:, -1] = box_save_tmp[:, -1] * -1
                                    # write_oriented_bbox(box_save_tmp, global_save_path + '_%04d_pred_box.ply' % maybe_idx)
                                    cv2.imwrite(im_path, tmp_im)
                                    np.save(box_path, tmp_save_box)
                                    # if im_path.find('008757_') != -1:
                                    #     print(1)
                                # print(1)
    ...
    def get_predicted_box_clip_embedding_2d_driven_objness(self, inputs, outputs, thres_obj=0.05, if_use_gt_box=False,
                                                           if_expand_box=False,
                                                           if_padding_input=True,
                                                           test=False,
                                                           curr_epoch=-1,
        ...
        gt_center_ori = outputs[
            "box_corners_xyz"]  # .clone().detach() #inputs["gt_box_corners_xyz"]  # shape: (8, 64, 8, 3) box_predictions["outputs"]['center_unnormalized']
        objectness_prob = outputs["objectness_prob"]
        box2d_guide_objness = outputs["2d_box_guide_objness"]
        # img_batch_name = inputs['im_name']
        # gt_angles = inputs["gt_box_angles"]
        # gt_sizes = inputs["gt_box_sizes"]
        # tmp = outputs['center_unnormalized']  # shape: (8, 128, 3)
        ...
        ####### to get the 2d box guide objectness label#######
        box2d_gt = inputs["box2d"]
        box2d_gt_present = inputs["box2d_present"]
        box2d_pseudo_label = torch.ones_like(outputs["2d_box_guide_objness"], dtype=torch.int64, device=self.device)
        if (not if_test) and (curr_epoch % self.online_nms_update_save_epoch == 0):  # and curr_epoch >= 540:
            for idx_batch in range(center_2dpoints.shape[0]):
                box2d_thisbatch = torch.zeros((center_2dpoints.shape[1], 4), device=self.device)
                scores = (objectness_prob[idx_batch] + box2d_guide_objness[idx_batch]) / 2.0
                del_box_list = []
                for idx_box in range(center_2dpoints.shape[1]):
                    box_size = outputs["size_unnormalized"][idx_batch][idx_box]
                    if torch.max(box_size) < 1e-16:
                ...
                box2d_gt_this_batch_tmp = box2d_gt[idx_batch][:, :-2]
                box2d_gt_present_this_batch = box2d_gt_present[idx_batch]
                box2d_gt_this_batch = torch.index_select(box2d_gt_this_batch_tmp, dim=0,
                                                         index=torch.nonzero(box2d_gt_present_this_batch)[:, 0])

                nms_after = torchvision.ops.nms(box2d_thisbatch, scores,
                                                iou_threshold=0.25)  # adopt the nms to decrease the number of boxes
                box2d_thisbatch_nms_select = torch.index_select(box2d_thisbatch, dim=0, index=nms_after)
                gt_predict_2d_iou = torchvision.ops.box_iou(box2d_thisbatch_nms_select, box2d_gt_this_batch)
                # gt_predict_2d_max_iou, gt_predict_2d_max_idx = torch.max(gt_predict_2d_iou, -1)
                if gt_predict_2d_iou.shape[0] == 0 or gt_predict_2d_iou.shape[1] == 0:
                    gt_predict_2d_max_iou = torch.zeros(gt_predict_2d_iou.shape[0], device='cuda')
                    # gt_predict_2d_max_idx = torch.zeros(gt_predict_2d_iou.shape[0], device='cuda')
                else:
                    gt_predict_2d_max_iou, gt_predict_2d_max_idx = torch.max(gt_predict_2d_iou, -1)
                # selected_idx = gt_predict_2d_max_idx[gt_predict_2d_max_iou > self.box2d_iou_thres]
                selected_2d_pos_idx = gt_predict_2d_max_iou > self.box2d_driven_iou
                nms_after = nms_after[selected_2d_pos_idx]
                box2d_pseudo_label[idx_batch][nms_after] = 0
        ...
        # # ################### add image embedding
        # full_image_proposal_features_batch = full_image_proposal_features_batch / full_image_proposal_features_batch.norm(dim=1, keepdim=True)
        outputs["box2d_pseudo_label"] = box2d_pseudo_label
        return outputs

    def get_predicted_box_clip_embedding_nms_iou_save_keep_clip_driven_with_cate_confidence_with_2d_pos_prior_combined_label(
            self,
            inputs,
            outputs,
            thres_obj=0.05,
            ...
            test=False,
            curr_epoch=-1,
            if_test=False, all_epoch=-1):  # the max number of classes are 18(train) and 14(val)
        gt_center_ori = outputs[
            "box_corners_xyz"]  # .clone().detach() #inputs["gt_box_corners_xyz"]  # shape: (8, 64, 8, 3) box_predictions["outputs"]['center_unnormalized']
        objectness_prob = outputs["objectness_prob"].detach().clone()
        K_tensor = inputs['K']
        ...
        # clip the point in the original size of the image
        text_features_clip = outputs["maybe_novel_text_features_clip"].to(torch.float32)
        # only_novel_text_features_clip = text_features_clip[:, 10:]
        # print(text_features_clip.shape)
        temperature_param = outputs["logit_scale"]
        for idx_batch in range(center_2dpoints.shape[0]):
            center_2dpoints[idx_batch, :, :, 0] = torch.clip(center_2dpoints[idx_batch, :, :, 0], min=0,
        ...
        if (not if_test) and (
            ...
            # get the scores of head embeddings
            text_correlation_embedding = outputs["text_correlation_embedding"]
            text_correlation_embedding = text_correlation_embedding / (
                    text_correlation_embedding.norm(dim=-1, keepdim=True) + 1e-32)
            head_text_features_clip = text_features_clip.unsqueeze(0).repeat(outputs["center_unnormalized"].shape[0], 1,
                                                                             1)
            correlation_map = torch.bmm(text_correlation_embedding,
                                        head_text_features_clip.permute(0, 2, 1)) * temperature_param
            head_scores = torch.nn.functional.softmax(correlation_map, dim=-1)[:, :, :]
            head_max_score, head_max_id = torch.max(head_scores, dim=-1)  # (8, 128)
            # head_max_id = head_max_id.unsqueeze(-1).repeat(1, 1, clip_scores.shape[-1])

        for idx_batch in range(center_2dpoints.shape[0]):
            if (not if_test) and (all_epoch == (self.begin_combine_score_epoch - 1)):
                novel_box_path = inputs['pseudo_box_path'][idx_batch]
                pseudo_bboxes_source_tmp = np.zeros((0, 8))
                np.save(novel_box_path,
                        pseudo_bboxes_source_tmp)  # re-init the boxes when arriave at idx_batch batch_number
            ...
            if (not if_test) and (curr_epoch % self.online_nms_update_save_epoch == 0):  # and curr_epoch >= 540:
                ...
                if gt_predict_2d_iou.shape[0] == 0 or gt_predict_2d_iou.shape[1] == 0:
                    ...
                    gt_predict_2d_max_iou, gt_predict_2d_max_idx = torch.max(gt_predict_2d_iou, -1)
                # selected_idx = gt_predict_2d_max_idx[gt_predict_2d_max_iou > self.box2d_iou_thres]
                selected_2d_pos_idx = gt_predict_2d_max_iou > self.box2d_iou_thres
                nms_after = nms_after[selected_2d_pos_idx]
                ...
                if gt_box_corners_nms_select.shape[0] > 0 and pred_box_corneres_nms_select.shape[0] > 0:
                    pred_box_corneres_nms_select_points = torch.zeros((pred_box_corneres_nms_select.shape[0], 6),
                                                                      device=self.device)
                    pred_box_corneres_nms_select_points[:, 0] = torch.min(pred_box_corneres_nms_select[:, :, 0], dim=1)[
                        0]
                ...
                for idx_box in novel_idx_save_final:
                    ...
                    idx_maybe_novel_box_list.append(idx_box)
                    head_max_score_list.append(head_max_score[idx_batch][idx_box])
                    head_max_id_list.append(head_max_id[idx_batch][idx_box])
                    head_score_list.append(head_scores[idx_batch][idx_box].unsqueeze(0))
                ...
                if len(proposal_maybe_novel_feature_list) > 0:
                    ...
                    # assert head_max_idx_array.shape[1] == maybe_max_idx.shape[1]

                    novel_idx_list_final_select = []
                    if all_epoch >= self.begin_combine_score_epoch:
                        head_max_score_array = torch.tensor(head_max_score_list, device=self.device)
                        head_max_idx_array = torch.tensor(head_max_id_list, device=self.device)
                        head_score_array = torch.cat(head_score_list, dim=0)
                        assert head_max_score_array.shape[0] == maybe_max_score.shape[0]
                        # assert head_max_score_array.shape[1] == maybe_max_score.shape[1]
                        assert head_max_idx_array.shape[0] == maybe_max_idx.shape[0]
                        if self.if_average_score:
                            assert head_score_array.shape == maybe_novel_scores.shape
                            average_scores = (head_score_array + maybe_novel_scores) / 2.0
                            combined_average_scores = torch.where(head_max_idx_array.unsqueeze(-1).repeat(1,
                                                                                                          average_scores.shape[
                                                                                                              -1]) >= self.train_range_max,
                                                                  average_scores, head_score_array)
                            combined_scores, combined_idx = torch.max(combined_average_scores, dim=-1)
                            # combined_idx = torch.where(head_max_idx_array >= self.train_range_max, combined_max_idx, head_max_idx_array)
                        else:
                            combined_scores = torch.where(head_max_idx_array >= self.train_range_max, maybe_max_score,
                                                          head_max_score_array)
                            combined_idx = torch.where(head_max_idx_array >= self.train_range_max, maybe_max_idx,
                                                       head_max_idx_array)
                    else:
                        combined_scores = maybe_max_score
                        combined_idx = maybe_max_idx
                    maybe_condition = torch.logical_and(combined_scores > self.clip_driven_keep_thres,
                                                        combined_idx >= self.train_range_max)
                    ...
                    if (curr_epoch % self.online_nms_update_save_epoch == 0):
                        for maybe_idx in range(maybe_condition.shape[0]):
                            ...
                            if maybe_item:
                                novel_idx_list_final_select.append(idx_maybe_novel_box_list[maybe_idx])
                                save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][7] = combined_idx[
                                    maybe_idx]
                                save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][8] = combined_scores[
                                    maybe_idx]
                                save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][9] = scores[
                                    idx_maybe_novel_box_list[maybe_idx]]  # update the novel box info
    ...
    def get_predicted_box_clip_embedding_with_2d_pos_prior_combined_label_2d_label(
            self,
            inputs,
            outputs,
            thres_obj=0.05,
        ...
        gt_center_ori = outputs[
            "box_corners_xyz"]  # .clone().detach() #inputs["gt_box_corners_xyz"]  # shape: (8, 64, 8, 3) box_predictions["outputs"]['center_unnormalized']
        objectness_prob = outputs["objectness_prob"].detach().clone()
        box2d_guide_objness = outputs["2d_box_guide_objness"].detach().clone()
        K_tensor = inputs['K']
        Rtilt_tensor = inputs['Rtilt']
        flip_array = inputs['flip_array'].unsqueeze(-1)
        scale_array = inputs['scale_array'].unsqueeze(1)
        ...
        for idx_batch in range(center_2dpoints.shape[0]):
            ...
            if (not if_test) and (curr_epoch % self.online_nms_update_save_epoch == 0):  # and curr_epoch >= 540:
                ...
                box2d_gt_this_batch_tmp = box2d_gt[idx_batch][:, :-2]
                box2d_gt_this_batch_cate_prob_tmp = box2d_gt[idx_batch][:, -2:]
                box2d_gt_present_this_batch = box2d_gt_present[idx_batch]

                box2d_gt_this_batch = torch.index_select(box2d_gt_this_batch_tmp, dim=0,
                                                         index=torch.nonzero(box2d_gt_present_this_batch)[:, 0])
                box2d_gt_this_batch_cate_prob = torch.index_select(box2d_gt_this_batch_cate_prob_tmp, dim=0,
                                                                   index=torch.nonzero(box2d_gt_present_this_batch)[:,
                                                                         0])
                ...
                if gt_predict_2d_iou.shape[0] == 0 or gt_predict_2d_iou.shape[1] == 0:
                    ...
                    gt_predict_2d_max_iou, gt_predict_2d_max_idx = torch.max(gt_predict_2d_iou, -1)
                # selected_idx = gt_predict_2d_max_idx[gt_predict_2d_max_iou > self.box2d_iou_thres]
                gt_predict_2d_max_iou_cate_prob = torch.index_select(box2d_gt_this_batch_cate_prob, dim=0,
                                                                     index=gt_predict_2d_max_idx)
                selected_2d_pos_idx = gt_predict_2d_max_iou > self.box2d_iou_thres
                nms_after = nms_after[selected_2d_pos_idx]
                cate_prob_after = gt_predict_2d_max_iou_cate_prob[selected_2d_pos_idx]
                ...
                if gt_box_corners_nms_select.shape[0] > 0 and pred_box_corneres_nms_select.shape[0] > 0:
                    ...
                    nms_after_iou_select = nms_after[select_boxes_idx]
                    cate_pro_after_iou_select = cate_prob_after[select_boxes_idx]
                else:
                    nms_after_iou_select = nms_after
                    cate_pro_after_iou_select = cate_prob_after
                ...
                box_save[idx_batch] = box_save[idx_batch].index_fill(dim=0, index=nms_after_iou_select, value=1)
                scores_after_nms = torch.index_select(scores, dim=0, index=nms_after_iou_select)
                cate_prob_after_objectness = cate_pro_after_iou_select[scores_after_nms > self.save_objectness]
                box_save[idx_batch][
                    scores < self.save_objectness] = 0  # set the objectness to keep, only keep the box which have larger objectness
                for idx_box in del_box_list:
                    box_save[idx_batch][idx_box] = 0
                ...
                if len(proposal_maybe_novel_feature_list) > 0:
                    ...
                    if all_epoch >= self.begin_combine_score_epoch:
                        ...
                        combined_scores = maybe_max_score
                        combined_idx = maybe_max_idx
                    # cate_prob_after_objectness
                    final_score = torch.maximum(cate_prob_after_objectness[:, -1], combined_scores)

                    cate_after_objectness = cate_prob_after_objectness[:, -2]
                    # for idx_box_tmp in range(cate_prob_after_objectness.shape[0]):
                    #     cate_after_objectness[idx_box_tmp] = cate_prob_after_objectness[idx_box_tmp, 0]
                    cate_after_objectness = cate_after_objectness.to(torch.int64)

                    final_idx = torch.where(cate_prob_after_objectness[:, -1] > combined_scores, cate_after_objectness,
                                            combined_idx)
                    maybe_condition = torch.logical_and(final_score > self.clip_driven_keep_thres,
                                                        final_idx >= self.train_range_max)
                    ...
                    if (curr_epoch % self.online_nms_update_save_epoch == 0):
                        for maybe_idx in range(maybe_condition.shape[0]):
                            ...
                            if maybe_item:
                                novel_idx_list_final_select.append(idx_maybe_novel_box_list[maybe_idx])
                                save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][7] = final_idx[
                                    maybe_idx]
                                save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][8] = final_score[
                                    maybe_idx]
                                save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][9] = scores[
                                    idx_maybe_novel_box_list[maybe_idx]]  # update the novel box info
    ...
    def get_virtual_embedding(self, inputs, outputs, thres_obj=0.05, if_use_gt_box=False,
                              if_expand_box=False,
                              if_padding_input=True,
                              test=False,
                              curr_epoch=-1):  # the max number of classes are 18(train) and 14(val)
        all_virtual_image = inputs["all_virtual_image"].detach().clone()
        bs_virtual = all_virtual_image.shape[0]
        all_virtual_image = all_virtual_image.reshape(-1, all_virtual_image.shape[2], all_virtual_image.shape[3],
                                                      all_virtual_image.shape[4])
        img_crop = all_virtual_image.permute(0, 3, 1, 2)
        img_crop_resized = self.resize(img_crop)
        image_proposal_batch = self.preprocess_for_tensor(img_crop_resized).unsqueeze(0)[0]  # change version
        proposal_features_batch = self.clip_model.encode_image(image_proposal_batch)  # change version
        if isinstance(proposal_features_batch, tuple):
            proposal_features_batch = proposal_features_batch[0]
        proposal_features_batch = proposal_features_batch.to(torch.float32).reshape(bs_virtual, -1,
                                                                                    proposal_features_batch.shape[1])
        outputs['all_virtual_image_embedding'] = proposal_features_batch

        # text_features_clip = outputs["text_features_clip"].to(torch.float32)
        # temperature_param = outputs["logit_scale"]
        # gt_image_embedding = proposal_features_batch / (
        #         proposal_features_batch.norm(dim=-1, keepdim=True) + 1e-32)
        # correlation_map = torch.bmm(gt_image_embedding,
        #                             text_features_clip.permute(0, 2, 1)) * temperature_param
        # box_scores = torch.nn.functional.softmax(correlation_map, dim=-1)
    ...
    def get_global_embedding(self, inputs, outputs, thres_obj=0.05, if_use_gt_box=False,
                              if_expand_box=False,
                              if_padding_input=True,
                              test=False,
                              curr_epoch=-1):  # the max number of classes are 18(train) and 14(val)
        all_virtual_image = inputs["all_global_image"].detach().clone()
        bs_virtual = all_virtual_image.shape[0]
        all_virtual_image = all_virtual_image.reshape(-1, all_virtual_image.shape[2], all_virtual_image.shape[3],
                                                      all_virtual_image.shape[4])
        img_crop = all_virtual_image.permute(0, 3, 1, 2)
        ...
        proposal_features_batch = proposal_features_batch.to(torch.float32).reshape(bs_virtual, -1,
                                                                                    proposal_features_batch.shape[1])
        outputs['all_global_image_embedding'] = proposal_features_batch
    ...
    def get_box_predictions(self, query_xyz, point_cloud_dims, box_features, point_clouds, inputs):
        ...
        # box2d_guide_logits = self.mlp_heads["box2d_guide_cls_head"](box_features).transpose(1, 2)
        ...
        # box2d_guide_logits = box2d_guide_logits.reshape(num_layers, batch, num_queries, -1)
        # box2d_guide_prob = torch.nn.functional.softmax(box2d_guide_logits, dim=-1)[..., 0]
        ...
        for l in range(num_layers):
            ...
            box_prediction = {
                ...
                # "2d_box_guide_objness": box2d_guide_prob[l],
                # "box2d_guide_logits": box2d_guide_logits[l],
    ...
    def forward(self, inputs, encoder_only=False, if_test=False, if_real_test=False, curr_epoch=-1, if_cmp_class=False,
        ...
        if (not if_real_test) and (not if_cmp_class) and (not if_test):  # do not calculate when testing
            if self.if_clip_superset:  # if adopt lvis superset
                ...
                box_predictions["outputs"]["text_features_clip"] = self.text_features_fg_norm[:self.train_range_max,
                                                                   :].unsqueeze(0).repeat(point_clouds.shape[0], 1,
                                                                                          1)  # text_features_clip
            ...
            if self.online_nms_update_save_novel_label_clip_driven_with_cate_confidence:
                if self.if_clip_superset:
                    ...
                    box_predictions["outputs"]["maybe_novel_text_features_clip"] = self.text_features_fg_norm[
                                                                                   ...
                                                                                   :]  # text_features_clip
                box_predictions[
                    "outputs"] = self.get_predicted_box_clip_embedding_nms_iou_save_keep_clip_driven_with_cate_confidence(
                    inputs,
                    box_predictions["outputs"], curr_epoch=curr_epoch, if_test=if_test)
                
            if self.online_3d_nod_global_novel:
                if self.if_clip_superset:
                    box_predictions["outputs"][
                        "maybe_novel_text_features_clip"] = self.superset_text_features_fg_norm
                else:
                    box_predictions["outputs"]["maybe_novel_text_features_clip"] = self.text_features_fg_norm[
                                                                                   :self.test_range_max,
                                                                                   :]  # text_features_clip
                box_predictions[
                    "outputs"] = self.get_3d_nod_global_novel(
                    inputs,
                    box_predictions["outputs"], curr_epoch=curr_epoch, if_test=if_test)

            elif self.online_nms_update_save_novel_label_clip_driven_with_cate_confidence_with_2d_pos_prior_combined_label:
                if self.if_clip_superset:
                    box_predictions["outputs"]["maybe_novel_text_features_clip"] = self.superset_text_features_fg_norm
                else:
                    box_predictions["outputs"]["maybe_novel_text_features_clip"] = self.text_features_fg_norm[
                                                                                   ...
                                                                                   :]  # text_features_clip
                box_predictions[
                    "outputs"] = self.get_predicted_box_clip_embedding_nms_iou_save_keep_clip_driven_with_cate_confidence_with_2d_pos_prior_combined_label(
                    inputs,
                    box_predictions["outputs"],
                    curr_epoch=curr_epoch,
                    if_test=if_test, all_epoch=all_epoch)
            elif self.online_nms_with_2d_pos_prior_combined_label_2d_label:
                if self.if_clip_superset:
                    box_predictions["outputs"]["maybe_novel_text_features_clip"] = self.superset_text_features_fg_norm
                else:
                    box_predictions["outputs"]["maybe_novel_text_features_clip"] = self.text_features_fg_norm[
                                                                                   ...
                                                                                   :]  # text_features_clip
                box_predictions[
                    "outputs"] = self.get_predicted_box_clip_embedding_with_2d_pos_prior_combined_label_2d_label(
                    inputs,
                    box_predictions["outputs"],
                    curr_epoch=curr_epoch,
                    if_test=if_test, all_epoch=all_epoch)
            elif self.adopt_2d_box_guide_objness:
                box_predictions["outputs"] = self.get_predicted_box_clip_embedding_2d_driven_objness(inputs,
                                                                                                     box_predictions[
                                                                                                         "outputs"],
                                                                                                     curr_epoch=curr_epoch)
            else:
        ...
        if if_real_test:
            ...
            # if self.ob_type == "mean":
            #     box_predictions["outputs"]["objectness_prob"] = (box_predictions["outputs"]["objectness_prob"] +
            #                                                      box_predictions["outputs"][
            #                                                          "2d_box_guide_objness"]) / 2.0
            # elif self.ob_type == "max":
            #     box_predictions["outputs"]["objectness_prob"] = torch.maximum(
            #         box_predictions["outputs"]["objectness_prob"], box_predictions["outputs"]["2d_box_guide_objness"])

        if self.if_get_virtual_embedding and (not if_real_test) and (not if_cmp_class) and (not if_test):
            box_predictions["outputs"] = self.get_virtual_embedding(inputs, box_predictions["outputs"],
                                                                    curr_epoch=curr_epoch)
        if self.if_get_global_embedding and (not if_real_test) and (not if_cmp_class) and (not if_test):
            box_predictions["outputs"] = self.get_global_embedding(inputs, box_predictions["outputs"],
                                                                    curr_epoch=curr_epoch)
...
class Model3DETRMultiClassHead(nn.Module):
    ...
    def __init__(
        ...
        # # #########################ResNet#########################
        self.clip_model, self.preprocess_for_tensor = clip.load("ViT-B/16", #"./CLIP/pretrain_models/RN50x64.pt", #"ViT-B/16", #"./CLIP/pretrain_models/RN50x64.pt", #"./CLIP/pretrain_models/ViT-L-14.pt",
                                                                device=self.device,
                                                                download_root='./CLIP/pretrain_models/',
                                                                if_transform_tensor=True)
        for name, param in self.clip_model.named_parameters():
    ...
    def forward(self, inputs, encoder_only=False, if_test=False, curr_epoch=-1, if_real_test=False, if_cmp_class=False, args=None):
        point_clouds = inputs["point_clouds"]
...
def build_3detr_predictedbox_distillation_head_2dobjectness(args, dataset_config):
    pre_encoder = build_preencoder(args)
    encoder = build_encoder(args) #build_image_text_encoder(args)
    decoder = build_decoder(args)
    # image_text_encoder = build_image_text_encoder(args)
    model = Model3DETRPredictedBoxDistillationHead2DObjectness(
        pre_encoder,
        encoder,
        decoder,
        dataset_config,
...
class Model3DETRPredictedBoxDistillationGlobalPool2DBoxHead(nn.Module):
    """
    Main 3DETR model. Consists of the following learnable sub-models
    - pre_encoder: takes raw point cloud, subsamples it and projects into "D" dimensions
                Input is a Nx3 matrix of N point coordinates
    ...
    def __init__(
        ...
        self.box_idx_list = np.arange(128, dtype=np.int8)
        self.if_adopt_caption_0313 = args.if_adopt_caption_0313
        self.if_adopt_caption_scannet_0415 = args.if_adopt_caption_scannet_0415
        
        self.if_use_clip_v2 = args.if_use_clip_v2
        ...
        if self.if_with_clip_train:
            ...
            print(len(all_classes_keys))
            # all_classes_keys.append('wall')
            # self.all_classes_keys = [ i.replace('_', ' ').lower()  for i in
            #                          all_classes_keys]
            # self.all_classes_keys = [i.replace('_', ' ').lower()  for i in
            #                          all_classes_keys]
            self.all_classes_keys = ['a photo of a ' + i.replace('_', ' ').lower() + ' in the scene' for i in
            ...
            self.all_classes_keys.append('a photo with just the background')
            ...
            if self.if_clip_superset:
                if self.if_adopt_caption_0313:
                    superset_all_classes = np.load(ALL_SUPERCLASS_PATH_Image2Caption, allow_pickle=True)
                elif self.if_adopt_caption_scannet_0415:
                    superset_all_classes = np.load(ALL_SUPERCLASS_PATH_Image2Caption_Scannet, allow_pickle=True)
                else:
                    superset_all_classes = np.load(ALL_SUPERCLASS_PATH_LVIS, allow_pickle=True)
                # superset_all_classes_keys = list(superset_all_classes)[1:]  # the  first one is "name"
                #
                ...
                if (not self.if_adopt_caption_0313) and (not self.if_adopt_caption_scannet_0415) and ALL_SUPERCLASS_PATH_LVIS.find('lvis_1204') != -1:
                    superset_all_classes_keys = list(superset_all_classes)[1:]  # the  first one is "name"
                    tmp_superset = ['a photo of a ' + i.replace('_', ' ').lower() + ' in the scene' for i in
                                    superset_all_classes_keys]
                else:
                    superset_all_classes_keys = list(superset_all_classes)
                    # tmp_superset = [i.replace('_', ' ').replace('/', ' or ').lower()
                    #                 for i in
                    #                 superset_all_classes_keys]
                    tmp_superset = ['a photo of a ' + i.replace('_', ' ').replace('/', ' or ').lower() + ' in the scene'
                                    for i in
                ...
                print('=====================class superset======================')
                # save_dic = {item: idx for idx, item in enumerate(self.superset_all_classes_keys)}
                self.superset_all_classes_keys.append('a photo with just the background')
                print(len(self.superset_all_classes_keys))
                assert len(self.superset_all_classes_keys) == 954 or len(self.superset_all_classes_keys) == 1201 or len(
                    self.superset_all_classes_keys) == 1216 or len(self.superset_all_classes_keys) == 232 or len(
                    self.superset_all_classes_keys) == 1203 or len(self.superset_all_classes_keys) == 369 or len(
                    self.superset_all_classes_keys) == 1387 or len(
                    self.superset_all_classes_keys) == 935 or len(
                    self.superset_all_classes_keys) == 1009 or len(
                    self.superset_all_classes_keys) == 1010
                print(self.superset_all_classes_keys)
                print(len(self.superset_all_classes_keys))

            print('=====================if use clip v2======================')
            print(self.if_use_clip_v2)
            if self.if_use_clip_v2:
                self.clip_model, self.preprocess_for_tensor = create_model_from_pretrained('ViT-B-16',
                                                                 pretrained='./open_clip/DFN2B-CLIP-ViT-B-16/open_clip_pytorch_model.bin',
                                                                 if_for_coda=True,
                                                                 device=self.device)
                tokenizer_clip_v2 = get_tokenizer('ViT-B-16')
                # self.clip_model, self.preprocess_for_tensor = create_model_from_pretrained('ViT-L-14',
                #                                                  pretrained='./open_clip/DFN2B-CLIP-ViT-L-14-39B/open_clip_pytorch_model.bin',
                #                                                  if_for_coda=True,
                #                                                  device=self.device)
                # tokenizer_clip_v2 = get_tokenizer('ViT-L-14')
            else:
                self.clip_model, self.preprocess_for_tensor = clip.load("ViT-B/16",
                                                                        # "./CLIP/pretrain_models/RN50x64.pt",
                                                                        # "./CLIP/pretrain_models/ViT-L-14.pt", #"./CLIP/pretrain_models/RN50x64.pt",
            ...
            if self.if_use_clip_v2:
                self.text = tokenizer_clip_v2(self.all_classes_keys, context_length=self.clip_model.context_length).to(self.device)
            else:
                self.text = clip.tokenize(self.all_classes_keys).to(self.device)
            ...
            self.all_cmp_classes_keys = ['a photo of a ' + i.replace('_', ' ').lower() + ' in the scene' for i in
                                         all_cmp_classes_keys]
            if self.if_use_clip_v2:
                self.cmp_text = tokenizer_clip_v2(self.all_cmp_classes_keys, context_length=self.clip_model.context_length).to(
                    self.device)
            else:
                self.cmp_text = clip.tokenize(self.all_cmp_classes_keys).to(self.device)
            ...
            if self.if_clip_superset:
                if self.if_use_clip_v2:
                    self.superset_text = tokenizer_clip_v2(self.superset_all_classes_keys,
                                                  context_length=self.clip_model.context_length).to(
                        self.device)
                else:
                    self.superset_text = clip.tokenize(self.superset_all_classes_keys).to(self.device)
                self.superset_text_features_fg = self.clip_model.encode_text(self.superset_text).to(torch.float32)
            ...
            # self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))  # .exp()
            if self.if_use_clip_v2:
                assert self.clip_model.visual.image_size[0] == self.clip_model.visual.image_size[1]
                self.resize = Resize(self.clip_model.visual.image_size[0],
                                     interpolation=torchvision.transforms.InterpolationMode.BICUBIC)
                scale_h = self.clip_model.visual.image_size[0] * 1.0 / dataset_config.image_size[0]
                scale_w = self.clip_model.visual.image_size[1] * 1.0 / dataset_config.image_size[0]
            else:
                self.resize = Resize(self.clip_model.visual.input_resolution,
                                     interpolation=torchvision.transforms.InterpolationMode.BICUBIC)
                scale_h = self.clip_model.visual.input_resolution * 1.0 / dataset_config.image_size[0]
            ...
            # # ############################ train use
            # if self.if_use_clip_v2:
            #     self.test_clip_model, self.test_preprocess_for_tensor =  create_model_from_pretrained('ViT-B-16',
            #                                                      pretrained='./open_clip/DFN2B-CLIP-ViT-B-16/open_clip_pytorch_model.bin',
            #                                                      if_for_coda=True,
            #                                                      device=self.device)
            #     tokenizer_clip_v2 = get_tokenizer('ViT-B-16')
            #     assert self.test_clip_model.visual.image_size[0] == self.test_clip_model.visual.image_size[1]
            #     self.test_resize = Resize(self.test_clip_model.visual.image_size[0],
            #                          interpolation=torchvision.transforms.InterpolationMode.BICUBIC)
            #     scale_h = self.test_clip_model.visual.image_size[0] * 1.0 / dataset_config.image_size[0]
            #     scale_w = self.test_clip_model.visual.image_size[0] * 1.0 / dataset_config.image_size[0]
            #     self.test_trans_mtx = torch.eye(2, device=self.device)
            #     self.test_trans_mtx[0, 0] *= scale_w
            #     self.test_trans_mtx[1, 1] *= scale_h
            # else:
            ...
            # if self.if_use_clip_v2:
            #     if len(self.all_classes_keys) == 233:
            #         self.test_text = tokenizer_clip_v2(self.all_classes_keys[:-1], context_length=self.test_clip_model.context_length).to(self.device)
            #     else:
            #         self.test_text = tokenizer_clip_v2(self.all_classes_keys[:-1], context_length=self.test_clip_model.context_length).to(self.device)
            # else:
            #     if len(self.all_classes_keys) == 233:
        ...
        print('=========================if online_3d_nod_global_novel_wo_denoising===============================')
        self.online_3d_nod_global_novel_wo_denoising = args.online_3d_nod_global_novel_wo_denoising
        print(self.online_3d_nod_global_novel_wo_denoising)
        
        print('=========================if online_3d_nod_global_novel_wo_denoising_simple_bg===============================')
        self.online_3d_nod_global_novel_wo_denoising_simple_bg = args.online_3d_nod_global_novel_wo_denoising_simple_bg
        print(self.online_3d_nod_global_novel_wo_denoising_simple_bg)
        
        
        print('=========================if online_3d_nod_global_novel_wo_bg_cate===============================')
        self.online_3d_nod_global_novel_wo_bg_cate = args.online_3d_nod_global_novel_wo_bg_cate
        print(self.online_3d_nod_global_novel_wo_bg_cate)
        ...
        print(self.adopt_2d_box_guide_objness)
        self.box2d_driven_iou = args.box2d_driven_iou
        self.box2d_driven_bg_iou = args.box2d_driven_bg_iou
        self.ob_type = args.ob_type
        print('=====================if_get_global_embedding========================')
        self.if_get_global_embedding = args.if_get_global_embedding
        print(self.if_get_global_embedding)
        print('=====================if_with_bg_cate=====================')
        self.if_with_bg_cate = args.if_with_bg_cate
        print(self.if_with_bg_cate)
    ...
    def get_3d_nod_global_novel(self, inputs, outputs,
        ...
        # clip the point in the original size of the image
        text_features_clip = outputs["maybe_novel_text_features_clip"].to(torch.float32)
        fg_text_features_clip = text_features_clip[:-1, :]
        # print(text_features_clip.shape)
        temperature_param = outputs["logit_scale"]
        for idx_batch in range(center_2dpoints.shape[0]):
            center_2dpoints[idx_batch, :, :, 0] = torch.clip(center_2dpoints[idx_batch, :, :, 0], min=0,
        ...
        box2d_gt = inputs["box2d"]
        box2d_gt_present = inputs["box2d_present"]
        # box2d_is_bg = torch.ones((center_2dpoints.shape[0], center_2dpoints.shape[1]), dtype=torch.int64, device=self.device)
        # if (not if_test) and (curr_epoch % self.online_nms_update_save_epoch == 0):  # and curr_epoch >= 540:
        #     for idx_batch in range(center_2dpoints.shape[0]):
        #         box2d_thisbatch = torch.zeros((center_2dpoints.shape[1], 4), device=self.device)
        #         scores = objectness_prob[idx_batch]
        ...
        #         selected_2d_pos_idx = gt_predict_2d_max_iou > self.box2d_driven_iou
        #         nms_after = nms_after[selected_2d_pos_idx]
        #         box2d_is_bg[idx_batch][nms_after] = 0
        #         print(1)
                # box2d_pseudo_label[idx_batch][nms_after] = 0
        ...
        for idx_batch in range(center_2dpoints.shape[0]):
            ...
            if (not if_test) and (curr_epoch % self.online_nms_update_save_epoch == 0):
                # use box2d_gt to denoise bg boxes
                nms_after = torchvision.ops.nms(box2d_thisbatch, scores,
                                                iou_threshold=0.25)  # adopt the nms to decrease the number of boxes
                box2d_thisbatch_nms_select = torch.index_select(box2d_thisbatch, dim=0, index=nms_after)
                gt_predict_2d_iou = torchvision.ops.box_iou(box2d_thisbatch_nms_select, box2d_gt_this_batch)
                ...
                if len(proposal_maybe_novel_feature_list) > 0:
                    ...
                    text_correlation_embedding = proposal_features_batch / (
                            proposal_features_batch.norm(dim=-1, keepdim=True) + 1e-32)
                    maybe_novel_correlation_map = torch.mm(text_correlation_embedding,
                                                           fg_text_features_clip.permute(1, 0)) * temperature_param
                    maybe_novel_scores = torch.nn.functional.softmax(maybe_novel_correlation_map, dim=-1)[:, :]
                    maybe_max_score, maybe_max_idx = torch.max(maybe_novel_scores, dim=-1)
                    novel_idx_list_final_select = []
                    maybe_condition = torch.logical_and(maybe_max_score > self.clip_driven_keep_thres,
                                                        maybe_max_idx >= self.train_range_max)
                    # maybe_condition = torch.logical_and(maybe_condition_1,
                    #                                     maybe_max_idx != (text_features_clip.shape[0]-1)) # not the bg cate
                    tmp_ori_pc = ori_pc_rgbd[idx_batch]
                    global_save_path = inputs['global_pool_path'][idx_batch]
                    # write_ply_rgb(tmp_ori_pc, global_save_path + '_all_pc.ply') # save global pool
                    if (not if_test) and (curr_epoch % self.online_nms_update_save_epoch == 0):
                        for maybe_idx in range(maybe_condition.shape[0]):
                            ...
                            if maybe_item:
                                ...
                                tmp_pred_box = tmp_pred_box_batch[idx_batch][idx_maybe_novel_box_list[maybe_idx]]
                                tmp_im = proposal_maybe_novel_feature_list[maybe_idx][0, :, :, :].permute(1, 2,
                                                                                                          0).detach().cpu().numpy()
                                box3d = flip_axis_to_depth(tmp_pred_box)
                                pc_tmp, _ = extract_pc_in_box3d(tmp_ori_pc, box3d)
                                ...
                                tmp_save_box = save_box_info[idx_batch, idx_maybe_novel_box_list[maybe_idx], :8]
                                if len(pc_tmp) > 500:  #
                                    box_path = global_save_path + '%09d_%04d_box.npy' % (
                                    maybe_idx + global_obj_num, device_id)
                                    pc_path = global_save_path + '%09d_%04d_pc.npy' % (
                                    maybe_idx + global_obj_num, device_id)
                                    im_path = global_save_path + '%09d_%04d_im.png' % (
                                    maybe_idx + global_obj_num, device_id)
                                    np.save(pc_path, pc_tmp)
                                    # write_ply_rgb(pc_tmp, global_save_path+'_%04d_pc.ply' % maybe_idx)
                                    # box_save_tmp = save_box_info[idx_batch][idx_maybe_novel_box_list[maybe_idx]][:7][np.newaxis, :]
            ...
            if (not if_test):
                sampled_box2d_thisbatch = box2d_thisbatch[idx_box_list]
                gt_predict_2d_iou = torchvision.ops.box_iou(sampled_box2d_thisbatch, box2d_gt_this_batch)
                if gt_predict_2d_iou.shape[0] == 0 or gt_predict_2d_iou.shape[1] == 0:
                    gt_predict_2d_max_iou = torch.zeros(gt_predict_2d_iou.shape[0], device='cuda')
                    # gt_predict_2d_max_idx = torch.zeros(gt_predict_2d_iou.shape[0], device='cuda')
                else:
                    gt_predict_2d_max_iou, gt_predict_2d_max_idx = torch.max(gt_predict_2d_iou, -1)
                # selected_idx = gt_predict_2d_max_idx[gt_predict_2d_max_iou > self.box2d_iou_thres]
                selected_2d_pos_idx = gt_predict_2d_max_iou < self.box2d_driven_bg_iou
                selected_2d_pos_idx_array = selected_2d_pos_idx.cpu().numpy()
                idx_box_list_array = np.array(idx_box_list)
                bg_idx_this_bath = idx_box_list_array[selected_2d_pos_idx_array]
                box_is_bg[idx_batch][bg_idx_this_bath] = 1

        if self.if_clip_weak_labels:
            text_features_clip = outputs["text_features_clip"].to(torch.float32)[:, :-1, :]
            temperature_param = outputs["logit_scale"]
            gt_image_embedding = outputs['gt_text_correlation_embedding']
            gt_image_embedding_mask = outputs['gt_text_correlation_embedding_mask']
            gt_image_embedding = gt_image_embedding / (
            ...
            weak_confidence_weight_clip = max_score
            bg_cate_weight_tensor = torch.ones_like(weak_confidence_weight_clip, device=self.device)
            weak_confidence_weight = torch.where(box_is_bg==1, bg_cate_weight_tensor, weak_confidence_weight_clip)
            weak_confidence_weight[
                gt_image_embedding_mask[:, :,
                0] < 1] = 0.0  # confience_weight for bg object is set to 0, because it is no
            
            weak_box_cate_label_clip = max_id
            bg_index = text_features_clip.shape[1]
            bg_cate_tensor = torch.ones_like(weak_box_cate_label_clip, device=self.device) * bg_index
            weak_box_cate_label = torch.where(box_is_bg==1, bg_cate_tensor, weak_box_cate_label_clip)
            outputs["weak_box_cate_label"] = weak_box_cate_label
            outputs["weak_confidence_weight"] = weak_confidence_weight
        return outputs

    def get_3d_nod_global_novel_wo_bg_cate(self, inputs, outputs,
                                thres_obj=0.05,
                                if_use_gt_box=False,
                                if_expand_box=False,
                                if_padding_input=True,
    ...
    def get_3d_nod_global_novel_wo_denoising(self, inputs, outputs,
                                thres_obj=0.05,
                                if_use_gt_box=False,
                                if_expand_box=False,
                                if_padding_input=True,
    ...
    def get_class_scores(self, box_predictions):
        ...
        text_features_clip = box_predictions["outputs"]["text_features_clip"].to(torch.float32)[:, :-1, :]
        # print(text_features_clip.shape)
        temperature_param = box_predictions["outputs"]["logit_scale"]
        text_correlation_embedding = outputs["text_correlation_embedding"]
        text_correlation_embedding = text_correlation_embedding / (
    ...
    def get_class_scores_with_bg_cate(self, box_predictions):
        '''
        calculate multi-class scores by clip-text-embedding
        '''
        if self.eval_layer_id != -1:
        ...
        box_predictions["outputs"]["sem_cls_prob"] = scores[:, :, :-1]
        class_bg = scores[:, :, -1]
        class_fg = 1.0 - class_bg
        ob_ness = box_predictions["outputs"]['objectness_prob'].clone().detach()
        box_predictions["outputs"]['objectness_prob'] = (ob_ness + class_fg) / 2.0
    ...
    def forward(self, inputs, encoder_only=False, if_test=False, if_real_test=False, curr_epoch=-1, if_cmp_class=False,
        ...
        if (not if_real_test) and (not if_cmp_class) and (not if_test):  # do not calculate when testing
            ...
            if self.online_3d_nod_global_novel:
            ...
            elif self.online_3d_nod_global_novel_wo_denoising:
                if self.if_clip_superset:
                    box_predictions["outputs"][
                        "maybe_novel_text_features_clip"] = self.superset_text_features_fg_norm
                else:
                    box_predictions["outputs"]["maybe_novel_text_features_clip"] = self.text_features_fg_norm[
                                                                                   ...
                                                                                   :]  # text_features_clip
                box_predictions[
                    "outputs"] = self.get_3d_nod_global_novel_wo_denoising(
                    inputs,
                    box_predictions["outputs"], curr_epoch=curr_epoch, if_test=if_test)
            elif self.online_3d_nod_global_novel_wo_denoising_simple_bg:
                if self.if_clip_superset:
                    box_predictions["outputs"][
                        "maybe_novel_text_features_clip"] = self.superset_text_features_fg_norm
                else:
                    box_predictions["outputs"]["maybe_novel_text_features_clip"] = self.text_features_fg_norm[
                                                                                   ...
                                                                                   :]  # text_features_clip
                box_predictions[
                    "outputs"] = self.get_3d_nod_global_novel_wo_denoising_simple_bg(
                    inputs,
                    box_predictions["outputs"], curr_epoch=curr_epoch, if_test=if_test)
            elif self.online_3d_nod_global_novel_wo_bg_cate:
                box_predictions["outputs"]["text_features_clip"] = box_predictions["outputs"]["text_features_clip"][:,
                                                                   :-1, :]
                ...
                if self.if_clip_superset:
                    ...
                    box_predictions["outputs"]["maybe_novel_text_features_clip"] = self.text_features_fg_norm[
                                                                                   ...
                                                                                   :]  # text_features_clip
                box_predictions[
                    "outputs"] = self.get_3d_nod_global_novel_wo_bg_cate(
                    inputs,
                    box_predictions["outputs"], curr_epoch=curr_epoch, if_test=if_test)
        ...
        if if_real_test:
            ...
            if self.if_with_clip:
                ...
                if self.if_with_fake_classes:
                    box_predictions, scores2, prob2 = self.get_class_scores_from_three(box_predictions)
                elif self.if_with_bg_cate:
                    box_predictions, scores2, prob2 = self.get_class_scores_with_bg_cate(box_predictions)
                    # box_predictions, scores2, prob2 = self.get_class_scores_sigmoid(box_predictions)
                else:
                    box_predictions, scores2, prob2 = self.get_class_scores(box_predictions)
            # if self.ob_type == "mean":
    ...
    def get_3d_nod_global_novel_wo_denoising_simple_bg(self, inputs, outputs,
                                thres_obj=0.05,
                                if_use_gt_box=False,
                                if_expand_box=False,
                                if_padding_input=True,
        ...
        if self.if_clip_weak_labels:
            ...
            weak_confidence_weight_clip = max_score
            # bg_cate_weight_tensor = torch.ones_like(weak_confidence_weight_clip, device=self.device)
            weak_confidence_weight = weak_confidence_weight_clip #torch.where(box_is_bg == 1, bg_cate_weight_tensor, weak_confidence_weight_clip)
            weak_confidence_weight[
                gt_image_embedding_mask[:, :,
                0] < 1] = 0.0  # confience_weight for bg object is set to 0, because it is no
            ...
            # bg_index = text_features_clip.shape[1]
            # bg_cate_tensor = torch.ones_like(weak_box_cate_label_clip, device=self.device) * bg_index
            weak_box_cate_label = weak_box_cate_label_clip #= torch.where(box_is_bg == 1, bg_cate_tensor, weak_box_cate_label_clip)
            outputs["weak_box_cate_label"] = weak_box_cate_label
            outputs["weak_confidence_weight"] = weak_confidence_weight
        return outputs

    
def build_3detr_predictedbox_distillation_globalpool_2dbox_head(args, dataset_config):
    pre_encoder = build_preencoder(args)
    encoder = build_encoder(args) #build_image_text_encoder(args)
    decoder = build_decoder(args)
    # image_text_encoder = build_image_text_encoder(args)
    model = Model3DETRPredictedBoxDistillationGlobalPool2DBoxHead(
        pre_encoder,
        encoder,
        decoder,
        dataset_config,
