...
from utils.eval_det import eval_det_multiprocessing, get_iou_obb, eval_det, eval_det_more
...
class APCalculator(object):
    ...
    def compute_metrics_more(self):
        ...
        for ap_iou_thresh in self.ap_iou_thresh:
            ...
            #     self.pred_map_cls, self.gt_map_cls, ovthresh=ap_iou_thresh
            # )
            rec, prec, ap, fpr, fnr, f1 = eval_det_more(
               self.pred_map_cls, self.gt_map_cls, ovthresh=ap_iou_thresh
            )
            # print(self.class2type_map)
            # for key in sorted(ap.keys()):
            ...
            fpr_list = []
            # print(ap.keys())
            for key in sorted(fpr.keys()):
                clsname = self.class2type_map[key] if self.class2type_map else str(key)
                try:
                    ret_dict["%s FPR" % (clsname)] = fpr[key]
                    fpr_list.append(fpr[key])
                except:
                    ret_dict["%s FPR" % (clsname)] = 0
                    fpr_list.append(0)
            ...
            for key in sorted(fpr.keys()):
                clsname = self.class2type_map[key] if self.class2type_map else str(key)
                try:
                    ret_dict["%s FNR" % (clsname)] = fnr[key]
                    fnr_list.append(fnr[key])
                except:
                    ret_dict["%s FNR" % (clsname)] = 0
                    fnr_list.append(0)
            ...
            for key in sorted(fpr.keys()):
                clsname = self.class2type_map[key] if self.class2type_map else str(key)
                try:
                    ret_dict["%s F1" % (clsname)] = f1[key]
                    f1_list.append(f1[key])
                except:
                    ret_dict["%s F1" % (clsname)] = 0
                    f1_list.append(0)

            if ap_vals.shape[0] > 2:
                if self.args.dataset_name.find('scannet') == -1 or ap_vals.shape[0] < 21:
                    ret_dict["FPR_fre"] = np.mean(fpr_list[:4])
                    ret_dict["FPR_common"] = np.mean(fpr_list[4:10])
                    ret_dict["FPR_base"] = np.mean(fpr_list[:10])
                    ret_dict["FPR_novel"] = np.mean(fpr_list[10:])
                    ret_dict["FPR"] = np.mean(fpr_list)
                else:
                    ret_dict["FPR_fre"] = np.mean(np.array(fpr_list)[self.dataset_config.seen_idx_list])
                    ret_dict["FPR_common"] = np.mean(np.array(fpr_list)[self.dataset_config.seen_idx_list])
                    ret_dict["FPR_base"] = np.mean(np.array(fpr_list)[self.dataset_config.seen_idx_list])
                    ret_dict["FPR_novel"] = np.mean(np.array(fpr_list)[self.dataset_config.novel_idx_list])
                    ret_dict["FPR"] = np.mean(fpr_list)
            else:
                ret_dict["FPR"] = np.mean(fpr_list)
            ...
            if ap_vals.shape[0] > 2:
                if self.args.dataset_name.find('scannet') == -1 or ap_vals.shape[0] < 21:
                    ret_dict["FNR_fre"] = np.mean(fnr_list[:4])
                    ret_dict["FNR_common"] = np.mean(fnr_list[4:10])
                    ret_dict["FNR_base"] = np.mean(fnr_list[:10])
                    ret_dict["FNR_novel"] = np.mean(fnr_list[10:])
                    ret_dict["FNR"] = np.mean(fnr_list)
                else:
                    ret_dict["FNR_fre"] = np.mean(np.array(fnr_list)[self.dataset_config.seen_idx_list])
                    ret_dict["FNR_common"] = np.mean(np.array(fnr_list)[self.dataset_config.seen_idx_list])
                    ret_dict["FNR_base"] = np.mean(np.array(fnr_list)[self.dataset_config.seen_idx_list])
                    ret_dict["FNR_novel"] = np.mean(np.array(fnr_list)[self.dataset_config.novel_idx_list])
                    ret_dict["FNR"] = np.mean(fnr_list)
            else:
                ret_dict["FNR"] = np.mean(fnr_list)
            ...
            if ap_vals.shape[0] > 2:
                if self.args.dataset_name.find('scannet') == -1 or ap_vals.shape[0] < 21:
                    ret_dict["F1_fre"] = np.mean(f1_list[:4])
                    ret_dict["F1_common"] = np.mean(f1_list[4:10])
                    ret_dict["F1_base"] = np.mean(f1_list[:10])
                    ret_dict["F1_novel"] = np.mean(f1_list[10:])
                    ret_dict["F1"] = np.mean(f1_list)
                else:
                    ret_dict["F1_fre"] = np.mean(np.array(f1_list)[self.dataset_config.seen_idx_list])
                    ret_dict["F1_common"] = np.mean(np.array(f1_list)[self.dataset_config.seen_idx_list])
                    ret_dict["F1_base"] = np.mean(np.array(f1_list)[self.dataset_config.seen_idx_list])
                    ret_dict["F1_novel"] = np.mean(np.array(f1_list)[self.dataset_config.novel_idx_list])
                    ret_dict["F1"] = np.mean(f1_list)
            else:
                ret_dict["F1"] = np.mean(f1_list)
    ...
    def metrics_to_str_more(self, overall_ret, per_class=True):
        # mAP_strs = []
        # AR_strs = []
        # prec_strs = []
        fpr_strs = []
        ...
        for ap_iou_thresh in self.ap_iou_thresh:
            ...
            fpr = overall_ret[ap_iou_thresh]["FPR"] * 100
            fpr_strs.append(f"FPR{ap_iou_thresh:.2f}: {fpr:.2f}\n")
            if "FPR_fre" in overall_ret[ap_iou_thresh].keys():
                FPR_fre = overall_ret[ap_iou_thresh]["FPR_fre"] * 100
                fpr_strs.append(f"FPR_fre{ap_iou_thresh:.2f}: {FPR_fre:.2f}\n")
                FPR_common = overall_ret[ap_iou_thresh]["FPR_common"] * 100
                fpr_strs.append(f"FPR_common{ap_iou_thresh:.2f}: {FPR_common:.2f}\n")
                FPR_base = overall_ret[ap_iou_thresh]["FPR_base"] * 100
                fpr_strs.append(f"FPR_base{ap_iou_thresh:.2f}: {FPR_base:.2f}\n")
                FPR_novel = overall_ret[ap_iou_thresh]["FPR_novel"] * 100
                fpr_strs.append(f"FPR_novel{ap_iou_thresh:.2f}: {FPR_novel:.2f}\n\n")

            fnr = overall_ret[ap_iou_thresh]["FNR"] * 100
            fnr_strs.append(f"FNR{ap_iou_thresh:.2f}: {fnr:.2f}\n")
            if "FNR_fre" in overall_ret[ap_iou_thresh].keys():
                FNR_fre = overall_ret[ap_iou_thresh]["FNR_fre"] * 100
                fnr_strs.append(f"FNR_fre{ap_iou_thresh:.2f}: {FNR_fre:.2f}\n")
                FNR_common = overall_ret[ap_iou_thresh]["FNR_common"] * 100
                fnr_strs.append(f"FNR_common{ap_iou_thresh:.2f}: {FNR_common:.2f}\n")
                FNR_base = overall_ret[ap_iou_thresh]["FNR_base"] * 100
                fnr_strs.append(f"FNR_base{ap_iou_thresh:.2f}: {FNR_base:.2f}\n")
                FNR_novel = overall_ret[ap_iou_thresh]["FNR_novel"] * 100
                fnr_strs.append(f"FNR_novel{ap_iou_thresh:.2f}: {FNR_novel:.2f}\n\n")

            f1 = overall_ret[ap_iou_thresh]["F1"] * 100
            f1_strs.append(f"F1{ap_iou_thresh:.2f}: {f1:.2f}\n")
            if "F1_fre" in overall_ret[ap_iou_thresh].keys():
                F1_fre = overall_ret[ap_iou_thresh]["F1_fre"] * 100
                f1_strs.append(f"F1_fre{ap_iou_thresh:.2f}: {F1_fre:.2f}\n")
                F1_common = overall_ret[ap_iou_thresh]["F1_common"] * 100
                f1_strs.append(f"F1_common{ap_iou_thresh:.2f}: {F1_common:.2f}\n")
                F1_base = overall_ret[ap_iou_thresh]["F1_base"] * 100
                f1_strs.append(f"F1_base{ap_iou_thresh:.2f}: {F1_base:.2f}\n")
                F1_novel = overall_ret[ap_iou_thresh]["F1_novel"] * 100
                f1_strs.append(f"F1_novel{ap_iou_thresh:.2f}: {F1_novel:.2f}\n\n")
        ...
        ap_str += ''.join(fpr_strs)
        ap_str += "\n"

        ap_str += ''.join(fnr_strs)
        ap_str += "\n"
    ...
    def metrics_to_dict(self, overall_ret):
        ...
        for ap_iou_thresh in self.ap_iou_thresh:
            ...
            if 'mAP_novel' in overall_ret[ap_iou_thresh].keys():
                metrics_dict[f"AP_novel_{ap_iou_thresh}"] = overall_ret[ap_iou_thresh]["mAP_novel"] * 100
                metrics_dict[f"AR_novel_{ap_iou_thresh}"] = overall_ret[ap_iou_thresh]["AR_novel"] * 100

            if 'mAP_base' in overall_ret[ap_iou_thresh].keys():
                metrics_dict[f"AP_base_{ap_iou_thresh}"] = overall_ret[ap_iou_thresh]["mAP_base"] * 100
                metrics_dict[f"AR_base_{ap_iou_thresh}"] = overall_ret[ap_iou_thresh]["AR_base"] * 100
        return metrics_dict
