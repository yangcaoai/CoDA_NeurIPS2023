...
class RandomCuboid(object):
    ...
    def __call__(self, point_cloud, target_boxes, per_point_labels=None, bboxes_seenclass=None, bboxes_seenconfi=None, all_virtual_image=None, is_virtual_box=None, num_boxes_before_virtual_aug=None, is_virtual_image=None, all_virtual_image_mask=None):
        ...
        for _ in range(100):
            ...
            if self.box_filter_policy == "center":
                ...
                if (
                    ...
                    # print(isinstance(bboxes_seenclass, np.ndarray))
                    # print(bboxes_seenclass.type)
                    if isinstance(bboxes_seenclass, np.ndarray):
                        bboxes_seenclass = bboxes_seenclass[keep_boxes]
                    if isinstance(bboxes_seenconfi, np.ndarray):
                        bboxes_seenconfi = bboxes_seenconfi[keep_boxes]
                    if isinstance(all_virtual_image, np.ndarray):
                        keep_novel_boxes = keep_boxes[num_boxes_before_virtual_aug:]
                        is_virtual_box= is_virtual_box[keep_boxes]
                        for idx_novel in range(keep_novel_boxes.shape[0]):
                            if not keep_novel_boxes[idx_novel]:
                                all_virtual_image[idx_novel, :, :, :] = 0
                                if isinstance(all_virtual_image_mask, np.ndarray):
                                    all_virtual_image_mask[idx_novel, :, :] = 0
                                is_virtual_image[idx_novel] = 0
                ...
                if isinstance(all_virtual_image, np.ndarray):
                    if isinstance(all_virtual_image_mask, np.ndarray):
                        return new_point_cloud, new_boxes, new_per_point_labels, bboxes_seenclass, bboxes_seenconfi, all_virtual_image, is_virtual_box, is_virtual_image, all_virtual_image_mask
                    else:
                        return new_point_cloud, new_boxes, new_per_point_labels, bboxes_seenclass, bboxes_seenconfi, all_virtual_image, is_virtual_box, is_virtual_image
                else:
                    if isinstance(bboxes_seenclass, np.ndarray):
                        if isinstance(bboxes_seenconfi, np.ndarray):
                            return new_point_cloud, new_boxes, new_per_point_labels, bboxes_seenclass, bboxes_seenconfi
                        else:
                            return new_point_cloud, new_boxes, new_per_point_labels, bboxes_seenclass
                    else:
        ...
        if isinstance(all_virtual_image, np.ndarray):
            if isinstance(all_virtual_image_mask, np.ndarray):
                return point_cloud, target_boxes, per_point_labels, bboxes_seenclass, bboxes_seenconfi, all_virtual_image, is_virtual_box, is_virtual_image, all_virtual_image_mask
            else:
                return point_cloud, target_boxes, per_point_labels, bboxes_seenclass, bboxes_seenconfi, all_virtual_image, is_virtual_box, is_virtual_image
        else:
            if isinstance(bboxes_seenclass, np.ndarray):
                if isinstance(bboxes_seenconfi, np.ndarray):
                    return point_cloud, target_boxes, per_point_labels, bboxes_seenclass, bboxes_seenconfi
                else:
                    return point_cloud, target_boxes, per_point_labels, bboxes_seenclass
            else:
