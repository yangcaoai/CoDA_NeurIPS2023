...
def polygon_clip(subjectPolygon, clipPolygon):
    ...
    def inside(p):
        # print('===============inside================')
        # print((cp2[0] - cp1[0]) * (p[1] - cp1[1]))
        # print((cp2[1] - cp1[1]) * (p[0] - cp1[0]))
        # print(cp1)
        # print(p)
        # print('===============inside================')
        return ((cp2[0] - cp1[0]) * (p[1] - cp1[1]) - (cp2[1] - cp1[1]) * (p[0] - cp1[0])) > 1e-12  # prevent the same value for gt box

    def onside(p):
        return abs((cp2[0] - cp1[0]) * (p[1] - cp1[1]) - (cp2[1] - cp1[1]) * (p[0] - cp1[0])) <= 1e-12 # for eval gt
    ...
    for clipVertex in clipPolygon:
        ...
        for subjectVertex in inputList: # subjectPolygon
            e = subjectVertex
            # print('=======================')
            # print(inside(s))
            # print(inside(e))
            ...
            # print(cp1)
            # print(cp2)
            if inside(e): # if subjectVertex in clipVertex
                if not inside(s):
            ...
            elif onside(s) and onside(e):  # eval for gt box
                outputList.append(e)
...
def convex_hull_intersection(p1, p2):
    ...
    if inter_p is not None:
        # print(inter_p)
        # len_inter_p = len(inter_p)
        # for idx in range(len_inter_p):
        #     print(inter_p[idx][0])
        #     print(inter_p[idx][1])
        #     if np.isnan(inter_p[idx][0]) and np.isnan(inter_p[idx][1]):
        #         print(inter_p[idx])
        #         del inter_p[idx]
        #
        # print(inter_p)
...
def get_3d_box_batch_np_xyz(box_size, angle, center):
    box_size = box_size / 2
    input_shape = angle.shape
    R = rotz_tensor(-angle)
    l = np.expand_dims(box_size[..., 0], -1)  # [x1,...,xn,1]
...
def get_3d_box_batch_tensor_xyz(box_size, angle, center):
