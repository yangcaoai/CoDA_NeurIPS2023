...
class SetCriterion(nn.Module):
    def __init__(self, matcher, dataset_config, loss_weight_dict, train_range_max=37, only_image_class=False, only_prompt_loss=False, args=None):
        ...
        semcls_2d_percls_weights = torch.ones(dataset_config.num_semcls + 1)
        semcls_2d_percls_weights[-1] = loss_weight_dict["loss_2d_no_object_weight"]
        self.register_buffer("semcls_2d_percls_weights", semcls_2d_percls_weights)
        ...
        del loss_weight_dict["loss_2d_no_object_weight"]
        if loss_weight_dict["loss_sem_cls_softmax_skip_none_gt_sample_weight"] > 0 or loss_weight_dict["loss_2d_sem_cls_softmax_skip_none_gt_sample_weight"] > 0 or loss_weight_dict["loss_sem_cls_softmax_skip_none_gt_sample_before_aug_weight"] > 0 or loss_weight_dict["loss_sem_cls_softmax_skip_none_gt_sample_en_discovery_objectness_weight"] > 0 or loss_weight_dict['loss_sem_cls_softmax_skip_none_gt_sample_keep_discovery_objectness_weight'] > 0:
            self.if_skip_no_seen_scene_objectness_sample = True
        else:
            self.if_skip_no_seen_scene_objectness_sample = False
        self.loss_2d_sem_cls_softmax_skip_none_gt_sample_weight = loss_weight_dict["loss_2d_sem_cls_softmax_skip_none_gt_sample_weight"]
        ...
        self.loss_functions = {
            ...
            "loss_2d_sem_cls_softmax_skip_none_gt_sample": self.loss_2d_sem_cls_softmax_skip_none_gt_sample,
            "loss_sem_cls_softmax_skip_none_gt_sample_before_aug": self.loss_sem_cls_softmax_skip_none_gt_sample_before_aug,
            ...
            "loss_predicted_region_embed_l1_virtual": self.loss_predicted_region_embed_l1_virtual,
            "loss_predicted_region_embed_l1_global": self.loss_predicted_region_embed_l1_global,
            "loss_predicted_region_embed_contrast_with_virtual_image": self.loss_predicted_region_embed_contrast_with_virtual_image,
            "loss_predicted_region_embed_contrast": self.loss_predicted_region_embed_contrast,
            ...
            "loss_feat_seen_softmax_weakly_loss_with_novel_cate_confi_skip_virtual": self.loss_feat_seen_softmax_weakly_loss_with_novel_cate_confi_skip_virtual,
            "loss_virtual_object_text_alignment": self.loss_virtual_object_text_alignment,
    ...
    def loss_2d_sem_cls_softmax_skip_none_gt_sample(self, outputs, targets, assignments):
        if self.loss_2d_sem_cls_softmax_skip_none_gt_sample_weight < 1e-6:
            res = torch.sum(outputs["box2d_guide_logits"]) * 0
            return {"loss_2d_sem_cls_softmax_skip_none_gt_sample": res}

        box2d_guide_logits = outputs["box2d_guide_logits"]
        gt_box_label = targets["box2d_pseudo_label"]
        loss = F.cross_entropy(
            box2d_guide_logits.transpose(2, 1),
            gt_box_label,
            self.semcls_2d_percls_weights,
            reduction="none",
        )
        final_loss = 0
        cnt_has_object = 0
        for batch_id in range(loss.shape[0]):
            num_obj_thisbatch = torch.sum(gt_box_label[batch_id]==0) # 0 is foreground
            if num_obj_thisbatch == 0:
                final_loss += (torch.sum(loss[batch_id]) * 0)
            else:
                final_loss += torch.sum(loss[batch_id])
        ...
        return {"loss_2d_sem_cls_softmax_skip_none_gt_sample": final_loss}

    def loss_sem_cls_softmax_skip_none_gt_sample_before_aug(self, outputs, targets, assignments):
        ...
        cnt_has_object = 0
        for batch_id in range(loss.shape[0]):
            num_obj_thisbatch = targets['num_boxes_before_virtual_aug'][batch_id]
            if num_obj_thisbatch == 0:
                final_loss += (torch.sum(loss[batch_id]) * 0)
            else:
                final_loss += torch.sum(loss[batch_id])
        ...
        return {"loss_sem_cls_softmax_skip_none_gt_sample_before_aug": final_loss}
    ...
    def loss_feat_seen_softmax_weakly_loss_with_novel_cate_confi_skip_virtual(self, outputs, targets, assignments):
        ...
        # gt_box_label[assignments["proposal_matched_mask"].int() == 0] = (correlation_map.shape[-1]-1)
        gt_box_label = torch.where(assignments["proposal_matched_mask"].int()>0, seen_matched_box_label, targets['weak_box_cate_label'])
        gt_box_label = torch.where(gt_box_label.int()!=-1, gt_box_label, targets['weak_box_cate_label'])
        if self.confidence_type=="clip-max-prob":
            gt_box_confidence_map = torch.where(assignments["proposal_matched_mask"].int()>0, seen_matched_confidence, targets['weak_confidence_weight'])
        elif self.confidence_type=="non-confidence":
            gt_box_confidence_map = torch.where(assignments["proposal_matched_mask"].int() > 0, seen_matched_confidence,
                                                targets['weak_confidence_weight'])
            gt_box_confidence_map = torch.where(gt_box_confidence_map.int() != -1, gt_box_confidence_map, targets['weak_confidence_weight'])
            gt_box_confidence_map[gt_box_confidence_map>1e-16] = 1
        # gt_box_confidence_map = targets['weak_confidence_weight'].detach().clone() #torch.where(assignments["proposal_matched_mask"].int()>0, seen_confidence, targets['weak_confidence_weight'])
        # gt_box_confidence_map[torch.logical_and(assignments["proposal_matched_mask"].int() > 0, gt_box_label !=-1)] = 1.0
        gt_box_confidence = gt_box_confidence_map
        ...
        all_num = torch.sum(gt_box_confidence > 1e-32) + 1e-32
        final_loss = torch.sum(loss * gt_box_confidence) / all_num
        return {"loss_feat_seen_softmax_weakly_loss_with_novel_cate_confi_skip_virtual": final_loss}

    def loss_virtual_object_text_alignment(self, outputs, targets, assignments):
        ...
        seen_matched_box_label = torch.gather(targets["gt_box_seen_sem_cls_label"], 1, assignments["per_prop_gt_inds"])
        seen_matched_confidence = torch.gather(targets["gt_box_seen_sem_cls_confi"], 1, assignments["per_prop_gt_inds"])
        gt_box_confidence_map = seen_matched_confidence
        # gt_box_label[assignments["proposal_matched_mask"].int() == 0] = (correlation_map.shape[-1]-1)
        gt_box_label = seen_matched_box_label #torch.where(assignments["proposal_matched_mask"].int()>0, , targets['weak_box_cate_label'])

        gt_box_confidence_map[seen_matched_confidence>1e-16] = 1
        # gt_box_confidence_map = targets['weak_confidence_weight'].detach().clone() #torch.where(assignments["proposal_matched_mask"].int()>0, seen_confidence, targets['weak_confidence_weight'])
        gt_box_confidence_map[assignments["proposal_matched_mask"].int() == 0] = 0
        gt_box_confidence = gt_box_confidence_map
        ...
        all_num = torch.sum(gt_box_confidence > 1e-32) + 1e-32
        final_loss = torch.sum(loss * gt_box_confidence) / all_num
        return {"loss_virtual_object_text_alignment": final_loss}
    ...
    def loss_predicted_region_embed_l1_virtual(self, outputs, targets, assignments):
        gt_text_correlation_embedding = targets['gt_text_correlation_embedding'] #torch.gather(
            #targets['gt_text_correlation_embedding'], 1, assignments["per_prop_gt_inds"].unsqueeze(-1).repeat(1, 1, targets['gt_text_correlation_embedding'].shape[2])
        #)
        # gt_text_correlation_embedding = gt_text_correlation_embedding / gt_text_correlation_embedding.norm(dim=-1, keepdim=True)
        ...
        # gt_text_correlation_embedding[assignments["proposal_matched_mask"].int() == 0] = correlation_map.shape[-1] - 1
        weight_maps = targets['gt_text_correlation_embedding_mask'].clone().detach() # torch.ones_like(assignments["proposal_matched_mask"], device='cuda')
        is_virtual_box = torch.gather(targets["is_virtual_box"], 1, assignments["per_prop_gt_inds"])
        is_virtual_box = is_virtual_box.clone().detach().unsqueeze(-1)
        is_virtual_box[assignments["proposal_matched_mask"].int() == 0] = 0 # is_virtual_box=1, only when the box is matched and the variable named 'is_virtual_box'=1
        weight_maps[is_virtual_box>0] = 0
        # weight_maps[assignments["proposal_matched_mask"].int() == 0] = 0.0
        # weight_maps = weight_maps.unsqueeze(-1)
        ave_weight = torch.sum(weight_maps)*text_correlation_embedding.shape[2]
        # pred_region_embed = outputs["region_embedding"]
        # target_region_embed = outputs["gt_region_embedding"]
        l1_loss_real = F.l1_loss(text_correlation_embedding*weight_maps, gt_text_correlation_embedding*weight_maps, reduction='sum') #/ ave_weight


        ##############
        # clip_weight_maps = targets['gt_text_correlation_embedding_mask']
        #
        # if clip_weight_maps.shape[-1] == 1:
        #     clip_weight_maps = clip_weight_maps[:, :, 0]
        # clip_weight_maps = clip_weight_maps.clone().detach()


        virtual_box_to_image_idx = torch.gather(targets["virtual_box_to_image_idx"], 1, assignments["per_prop_gt_inds"])
        all_virtual_image_embedding = targets["all_virtual_image_embedding"]
        # all_virtual_image_embedding = all_virtual_image_embedding / (
        #             all_virtual_image_embedding.norm(dim=-1, keepdim=True) + 1e-32)
        all_virtual_image_embedding_select = torch.gather(all_virtual_image_embedding, 1, virtual_box_to_image_idx.unsqueeze(-1).repeat(1,1,all_virtual_image_embedding.shape[-1]))
        ave_weight_virtual = torch.sum(is_virtual_box) * text_correlation_embedding.shape[2]
        l1_loss_virtual = F.l1_loss(text_correlation_embedding * is_virtual_box, all_virtual_image_embedding_select * is_virtual_box, reduction='sum') #/ ave_weight_virtual
        l1_loss = (l1_loss_virtual+l1_loss_real) / (ave_weight_virtual+ave_weight)
        # virtual_correlation_map = torch.bmm(text_correlation_embedding,
        #                             all_virtual_image_embedding_select.permute(0, 2, 1)) * temperature_param
        # loss_virtual = F.cross_entropy(
        #     virtual_correlation_map.transpose(2, 1),
        #     label_clip,
        #     reduction="none",
        # )
        # print(l1_loss.shape)
        return {"loss_predicted_region_embed_l1_virtual": l1_loss}

    def loss_predicted_region_embed_l1_global(self, outputs, targets, assignments):
        gt_text_correlation_embedding = targets['gt_text_correlation_embedding']  # torch.gather(
        # targets['gt_text_correlation_embedding'], 1, assignments["per_prop_gt_inds"].unsqueeze(-1).repeat(1, 1, targets['gt_text_correlation_embedding'].shape[2])
        # )
        # gt_text_correlation_embedding = gt_text_correlation_embedding / gt_text_correlation_embedding.norm(dim=-1, keepdim=True)
        ...
        # gt_text_correlation_embedding[assignments["proposal_matched_mask"].int() == 0] = correlation_map.shape[-1] - 1
        weight_maps = targets[
            'gt_text_correlation_embedding_mask'].clone().detach()  # torch.ones_like(assignments["proposal_matched_mask"], device='cuda')
        is_global_box = torch.gather(targets["is_global_box"], 1, assignments["per_prop_gt_inds"])
        is_global_box = is_global_box.clone().detach().unsqueeze(-1)
        is_global_box[assignments[
                          "proposal_matched_mask"].int() == 0] = 0  # is_global_box=1, only when the box is matched and the variable named 'is_global_box'=1
        weight_maps[is_global_box > 0] = 0
        # weight_maps[assignments["proposal_matched_mask"].int() == 0] = 0.0
        # weight_maps = weight_maps.unsqueeze(-1)
        ave_weight = torch.sum(weight_maps) * text_correlation_embedding.shape[2]
        # pred_region_embed = outputs["region_embedding"]
        # target_region_embed = outputs["gt_region_embedding"]
        l1_loss_real = F.l1_loss(text_correlation_embedding * weight_maps, gt_text_correlation_embedding * weight_maps,
                                 reduction='sum')  # / ave_weight
        ...
        global_box_to_image_idx = torch.gather(targets["global_box_to_image_idx"], 1, assignments["per_prop_gt_inds"])
        all_global_image_embedding = targets["all_global_image_embedding"]
        # all_global_image_embedding = all_global_image_embedding / (
        #             all_global_image_embedding.norm(dim=-1, keepdim=True) + 1e-32)
        all_global_image_embedding_select = torch.gather(all_global_image_embedding, 1,
                                                         global_box_to_image_idx.unsqueeze(-1).repeat(1, 1,
                                                                                                      all_global_image_embedding.shape[
                                                                                                          -1]))
        ave_weight_global = torch.sum(is_global_box) * text_correlation_embedding.shape[2]
        l1_loss_global = F.l1_loss(text_correlation_embedding * is_global_box,
                                   all_global_image_embedding_select * is_global_box,
                                   reduction='sum')  # / ave_weight_global
        l1_loss = (l1_loss_global + l1_loss_real) / (ave_weight_global + ave_weight)
        # global_correlation_map = torch.bmm(text_correlation_embedding,
        #                             all_global_image_embedding_select.permute(0, 2, 1)) * temperature_param
        # loss_global = F.cross_entropy(
        #     global_correlation_map.transpose(2, 1),
        #     label_clip,
        #     reduction="none",
        # )
        # print(l1_loss.shape)
        return {"loss_predicted_region_embed_l1_global": l1_loss}

    def loss_predicted_region_embed_contrast_with_virtual_image(self, outputs, targets, assignments):
        if "all_virtual_image_embedding" not in targets.keys():
            return {"loss_predicted_region_embed_contrast_with_virtual_image": 0.0}
        text_correlation_embedding = outputs["text_correlation_embedding"]
        text_correlation_embedding = text_correlation_embedding / (
                    text_correlation_embedding.norm(dim=-1, keepdim=True) + 1e-32)

        gt_text_correlation_embedding = targets['gt_text_correlation_embedding']
        gt_text_correlation_embedding = gt_text_correlation_embedding / (
                    gt_text_correlation_embedding.norm(dim=-1, keepdim=True) + 1e-32)

        # print(text_features_clip.shape)
        temperature_param = targets["logit_scale"]
        clip_correlation_map = torch.bmm(text_correlation_embedding,
                                    gt_text_correlation_embedding.permute(0, 2, 1)) * temperature_param
        # clip_scores = torch.nn.functional.softmax(clip_correlation_map, dim=-1)[:, :, :]
        clip_weight_maps = targets['gt_text_correlation_embedding_mask']
        is_virtual_box = torch.gather(targets["is_virtual_box"], 1, assignments["per_prop_gt_inds"])
        is_virtual_box[assignments["proposal_matched_mask"].int() == 0] = 0 # is_virtual_box=1, only when the box is matched and the variable named 'is_virtual_box'=1
        clip_weight_maps[is_virtual_box>0] = 0
        if clip_weight_maps.shape[-1] == 1:
            clip_weight_maps = clip_weight_maps[:, :, 0]
        clip_weight_maps = clip_weight_maps.clone().detach()
        is_virtual_box = is_virtual_box.clone().detach()
        label_clip = torch.unsqueeze(torch.arange(clip_correlation_map.shape[1], device='cuda'), dim=0).repeat(clip_weight_maps.shape[0], 1) # may be multi-class contrastive loss
        loss_clip = F.cross_entropy(
            clip_correlation_map.transpose(2, 1),
            label_clip,
            reduction="none",
        )
        ...
        sum_loss = torch.sum(clip_weight_maps * loss_clip + is_virtual_box *  loss_virtual) / torch.sum(clip_weight_maps + is_virtual_box)
        return {"loss_predicted_region_embed_contrast_with_virtual_image": sum_loss}


    def loss_predicted_region_embed_contrast(self, outputs, targets, assignments):
        text_correlation_embedding = outputs["text_correlation_embedding"]
        text_correlation_embedding = text_correlation_embedding / (
                    text_correlation_embedding.norm(dim=-1, keepdim=True) + 1e-32)
        ...
        #     reduction="none",
        # )
        sum_loss = torch.sum(clip_weight_maps * loss_clip ) / torch.sum(clip_weight_maps)
        return {"loss_predicted_region_embed_contrast": sum_loss}
    ...
    def forward(self, outputs, targets):
        ...
        if 'novel_box_judge' in outputs["outputs"].keys():
            targets["novel_box_judge"] = outputs["outputs"]['novel_box_judge']
        if 'all_virtual_image_embedding' in outputs["outputs"].keys():
            targets['all_virtual_image_embedding'] = outputs["outputs"]['all_virtual_image_embedding'].detach().clone()
        if 'all_global_image_embedding' in outputs["outputs"].keys():
            targets['all_global_image_embedding'] = outputs["outputs"]['all_global_image_embedding'].detach().clone()
        if "box2d_pseudo_label" in outputs["outputs"].keys():
            targets["box2d_pseudo_label"] = outputs["outputs"]["box2d_pseudo_label"].detach().clone()
...
def build_criterion(args, dataset_config):
    if args.only_image_class:
        ...
        loss_weight_dict = {
            ...
            "loss_2d_sem_cls_softmax_skip_none_gt_sample_weight": args.loss_2d_sem_cls_softmax_skip_none_gt_sample_weight,
            "loss_sem_cls_softmax_skip_none_gt_sample_before_aug_weight": args.loss_sem_cls_softmax_skip_none_gt_sample_before_aug_weight,
            ...
            "loss_no_object_weight": args.loss_no_object_weight,
            "loss_2d_no_object_weight": args.loss_2d_no_object_weight,
            ...
            "loss_predicted_region_embed_l1_virtual_weight": args.loss_predicted_region_embed_l1_virtual_weight,
            "loss_predicted_region_embed_l1_global_weight": args.loss_predicted_region_embed_l1_global_weight,
            "loss_predicted_region_embed_contrast_with_virtual_image_weight": args.loss_predicted_region_embed_contrast_with_virtual_image_weight,
            "loss_predicted_region_embed_contrast_weight": args.loss_predicted_region_embed_contrast_weight,
            ...
            "loss_feat_seen_softmax_weakly_loss_with_novel_cate_confi_skip_virtual_weight": args.loss_feat_seen_softmax_weakly_loss_with_novel_cate_confi_skip_virtual_weight,
            "loss_virtual_object_text_alignment_weight": args.loss_virtual_object_text_alignment_weight,
