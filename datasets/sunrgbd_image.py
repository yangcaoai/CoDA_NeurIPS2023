...
if here_path.find('user-job-dir')!=-1:
    ...
    CLIP_FEAT_PATH_V1 = "/cache/data/sunrgbd_v1_revised_0415/save_clip_feat/sunrgbd_clip_feat"
    ...
    CLIP_FEAT_PATH_V1 = "Data/3d_indoor/sunrgb_d/sunrgbd_v1_revised_0415/save_clip_feat/sunrgbd_clip_feat"
...
class SunrgbdImageDatasetConfig(object):
   ...
   #      self.test_range = [i for i in range(690)] # test range must be larger than train range, which is because it contains unseen class
     ...
     #       'box':37, 'whiteboard':38, 'person':39, 'night_stand':40, 'bag':41} 
        ...
        self.eval_class2type = {self.type2class[t]: t for t in self.type2class}
...
class SunrgbdImageDetectionDataset(Dataset):
    def __init__(
        ...
        assert split_set in ["train", "val", "trainval", "minival", "noveltrain", "novelval", "toiletval", "printerval", "toilettrain", "tableval", "endtableval", "coffeetableval", "papershow"]
        ...
        clip_feat_dir = CLIP_FEAT_PATH_V1
        print('CLIP feat dir:')
        print(clip_feat_dir)
        self.clip_feat_dir = clip_feat_dir + "_%s" % (split_set)
        self.if_load_clip_feat = dataset_config.args.if_load_clip_feat
        print('---------------if_load_clip_feat---------------')
        print(self.if_load_clip_feat)

        # print(self.data_path)
        self.calib_path = CALIB_PATH
        self.image_path = IMAGE_PATH

        if split_set in ["train", "val", "minival", "noveltrain", "novelval", "toiletval", "printerval", "toilettrain", "tableval", "endtableval", "coffeetableval", "papershow"]:
            ...
            # self.scan_names_2 = []
            # for tmp in self.scan_names:
            #     if int(tmp) % 10 == 0:
            #         self.scan_names_2.append(tmp)
            # self.scan_names = self.scan_names_2[:-299]
    ...
    def __getitem__(self, idx):
        ...
        if self.if_load_clip_feat:
            self.feat_name = os.path.join(self.clip_feat_dir, scan_name + '_clip_feat.npy')
            clip_feat = np.load(self.feat_name)
