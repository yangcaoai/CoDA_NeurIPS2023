...
class SunrgbdImageCmpDatasetConfig(object):
    def __init__(self, if_print=True, args=None, use_v1=True):
        if args is not None:
            self.num_semcls = 20
            self.use_v1 = args.if_use_v1
        else:
            self.num_semcls = 20 #137 #690 #1140 #10 #42 #10
            self.use_v1 = use_v1
   ...
   #      self.test_range = [i for i in range(690)] # test range must be larger than train range, which is because it contains unseen class
        if args is not None:
            ...
            self.test_range = [0, 1, 2, 4, 5, 6, 9, 11, 14, 22, 24, 27, 31, 40, 48, 51, 55, 71, 106, 218] #[i for i in range(args.test_range_min, args.test_range_max)]
        else:
            ...
            self.test_range = [0, 1, 2, 4, 5, 6, 9, 11, 14, 22, 24, 27, 31, 40, 48, 51, 55, 71, 106, 218]  # [i for i in range(232)] # test range must be larger than train range, which is because it contains unseen class
        self.test_class_to_dix = {0:0, 1:1, 2:2, 4:3, 5:4, 6:5, 9:6, 11:7, 14:8, 22:9, 24:10, 27:11, 31:12, 40:13, 48:14, 51:15, 55:16, 71:17, 106:18, 218:19}
        if if_print:
     ...
     #       'box':37, 'whiteboard':38, 'person':39, 'night_stand':40, 'bag':41} 
        self.class2type_former = {self.type2class[t]: t for t in self.type2class}
        # print(self.class2type_former)
        # print(self.type2class.values())
        self.class2type = {}
        for t in self.class2type_former.keys():
            if t in self.test_class_to_dix.keys():
                self.class2type[self.test_class_to_dix[t]] = self.class2type_former[t]
...
class SunrgbdImageCmpDetectionDataset(Dataset):
    def __init__(
        self,
        dataset_config,
        split_set="train",
        ...
        assert split_set in ["train", "val", "trainval", "minival", "noveltrain", "novelval", "toiletval", "toilettrain", "tableval", "endtableval", "coffeetableval"]
        ...
        if split_set in ["train", "val", "minival", "noveltrain", "novelval", "toiletval", "toilettrain", "tableval", "endtableval", "coffeetableval"]:
