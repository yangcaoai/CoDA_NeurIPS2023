...
from utils.box_util import (flip_axis_to_camera_np, flip_axis_to_camera_tensor,
                            get_3d_box_batch_np, get_3d_box_batch_tensor, get_3d_box_batch_tensor_xyz)
...
class ScannetAnonymousDatasetConfig(object):
    ...
    def box_parametrization_to_corners_xyz(self, box_center_unnorm, box_size, box_angle):
        # box_center_upright = flip_axis_to_camera_tensor(box_center_unnorm)
        boxes = get_3d_box_batch_tensor_xyz(box_size, -1 * box_angle, box_center_unnorm) # here should have a bug, cause I have * -1
        return boxes
...
class ScannetDetectionAnonymousDataset(Dataset):
    def __init__(
            self,
            dataset_config,
            split_set="train",
