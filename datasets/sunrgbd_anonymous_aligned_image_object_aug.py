...
if here_path.find('user-job-dir')!=-1:
    ...
    ALL_VIRTUAL_OBJECT_PATH = "/cache/data/sunrgbd_v1_revised_0415/virtual_object_points_by_point_e/"
else:
    ...
    ALL_SUPERCLASS_PATH = "datasets/all_classes_trainval_v2_revised_del_val_less_than_5_classes_lvis_combined_1201classes.npy"
    ALL_VIRTUAL_OBJECT_PATH = "/home/xuhang/yang/codes/3d_generation/point-e-main/point_e/examples/virtual_object_points_by_point_e/"
...
class SunrgbdAnonymousAlignedImageObjectAugDatasetConfig(object):
...
class SunrgbdAnonymousAlignedImageObjectAugDetectionDataset(Dataset):
    def __init__(
        self,
        dataset_config,
        split_set="train",
        ...
        assert split_set in ["train", "val", "trainval", "minival", "testtrain", "toilettrain"]
        ...
        if split_set in ["train", "val", "minival", "testtrain", "toilettrain"]:
        ...
        self.virtual_objs = glob.glob(ALL_VIRTUAL_OBJECT_PATH+'*.npy')
    ...
    def random_move(self, point_cloud_ori, virtual_obj_ori):
        '''
    ...
    def __getitem__(self, idx):
        ...
        bboxes_source = np.load(scan_path + "_bbox.npy")  # K,8
        ...
        for idx_box in range(bboxes.shape[0]):
            ...
            if (bboxes[idx_box, -1] in self.dataset_config.test_range): # and (20 > bboxes[idx_box, -1]):
                if (bboxes[idx_box, -1] in self.dataset_config.train_range):
                    bboxes_seenclass.append(bboxes[idx_box, -1])
                else:
                    bboxes_seenclass.append(self.dataset_config.train_max)
                bboxes[idx_box, -1] = 0 #bboxes[idx_box, -1] - self.dataset_config.test_range[0] # begin from zero
                cnt_bboxes += 1
        ...
        if self.split_set == 'train' or 'toilettrain':
            ...
            virtual_obj_path = self.virtual_objs[virtual_idx]
            virtual_obj_class = os.path.basename(virtual_obj_path)[:-9]
            assert virtual_obj_class in self.dataset_config.type2class.keys()
            virtual_obj_class_id = self.dataset_config.type2class[virtual_obj_class]
            virtual_obj = np.load(virtual_obj_path)

            virtual_obj_moved = self.random_move(point_cloud, virtual_obj)
            virtual_obj_rotated, virtual_boxes = self.get_boxes(virtual_obj_moved, virtual_obj_class_id)
            while self.check_conclusion(point_cloud, virtual_boxes):
                virtual_obj_moved = self.random_move(point_cloud, virtual_obj)
                virtual_obj_rotated, virtual_boxes = self.get_boxes(virtual_obj_moved, virtual_obj_class_id)
            point_cloud = np.concatenate((point_cloud, virtual_obj_rotated), axis=0)
            # np.save('debug_combined_pc', combined_pc)
            # ###debuging by saving points
            # write_ply(point_cloud, 'debug_combined_pc_moved_3.ply')
            # virtual_boxes[:, 6] = -1 * virtual_boxes[:, 6]
            # write_oriented_bbox(virtual_boxes, 'debug_virtual_boxes_3.ply')
            # ###debuging by saving points
...
# ################skip the no-seen-object scene###################
        ...
        uv_2d, _, _ = calib.project_upright_depth_to_image(point_cloud[:, :3])
        ...
        if self.augment:
            ...
            if self.use_random_cuboid:  # True
                point_cloud, bboxes, _, bboxes_seenclass = self.random_cuboid_augmentor(
                    point_cloud, bboxes, bboxes_seenclass=bboxes_seenclass
                )
